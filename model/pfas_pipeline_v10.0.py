...
Changes in v10.0 — Apparent half-life estimation (exploratory)
---------------------------------------------------
...
- EXPLORATORY / DIAGNOSTIC ONLY, not a headline model result: two time
  points give a single decay line with no residual to check and no
  confidence interval derivable from the data. Values are labeled
  "apparent half-life (2-point estimate)" and displayed rounded to whole
  years (not decimals) so the output doesn't imply more precision than
  n=2 can support.
...
def compute_apparent_half_life(nhanes_df, pfas_col="PFAS_Name",
    ...
    """
    Apparent half-life (2-point estimate) per PFAS from NHANES wave medians.

    EXPLORATORY / DIAGNOSTIC ONLY — not a headline model result on the same
    footing as the R² figures elsewhere in this pipeline.

    CAVEAT: cross-sectional, not longitudinal, AND built from exactly two
    time points. Two points determine one line -- there is no way to check
    whether first-order decay actually holds, no residual to inspect, and
    no confidence interval derivable from the data (n=2 gives zero degrees
    of freedom for error estimation). Values should be read as order-of-
    magnitude signals, not precise measurements, and are deliberately
    rounded to whole years at display time for that reason.

    Also conflates true biological elimination with declining population
    exposure and cohort effects between survey waves. Treat as a rough
    literature sanity-check and exposure-trend signal, not a clinical or
    regulatory elimination rate.
    """
...
def _fmt_half_life(years):
    """
    Round a half-life for display without implying false precision.
    Whole years for values >=1y. For sub-year values (e.g. PFBS's ~1-month
    literature half-life), whole-year rounding would misleadingly show "0y" --
    switch to whole months instead so fast-clearing compounds aren't hidden.
    """
    if pd.isna(years):
        return "n/a"
    if abs(years) >= 1:
...
def plot_half_life_comparison(hl_df, path):
    """
    Bar chart: NHANES apparent half-life (2-point estimate) vs. published
    literature half-life per PFAS. EXPLORATORY DIAGNOSTIC, not a headline
    model result -- see compute_apparent_half_life() docstring. Values are
    deliberately rounded to whole years since n=2 cannot support more
    precision than that.
    """
    ...
    fig, ax = plt.subplots(figsize=(8, max(4.5, len(plot_df) * 0.65)))
    bars_nhanes = ax.barh(y - 0.18, plot_df["half_life_years"], height=0.35, color="steelblue",
            label="NHANES apparent half-life\n(2-point estimate, exploratory)")
    bars_lit = ax.barh(y + 0.18, plot_df["literature_half_life_years"], height=0.35, color="coral",
            label="Published literature half-life\n(longitudinal cohort studies)")

    # Value labels via _fmt_half_life: whole years, or whole months for
    # sub-year values (e.g. PFBS) so fast-clearing compounds aren't shown as "0y"
    for bar, val in zip(bars_nhanes, plot_df["half_life_years"]):
        ax.text(bar.get_width() + max(plot_df["half_life_years"]) * 0.01,
                bar.get_y() + bar.get_height() / 2, f"~{_fmt_half_life(val)}",
                va="center", fontsize=8, color="steelblue")
    for bar, val in zip(bars_lit, plot_df["literature_half_life_years"]):
        ax.text(bar.get_width() + max(plot_df["half_life_years"]) * 0.01,
                bar.get_y() + bar.get_height() / 2, _fmt_half_life(val),
                va="center", fontsize=8, color="coral")

    ax.set_yticks(y)
    ax.set_yticklabels(plot_df["PFAS_Name"])
    ax.set_xlabel("Half-life (years)")
    ax.set_title("Apparent Half-Life (2-Point Estimate) vs. Literature\n"
                "Exploratory diagnostic — cross-sectional, n=2 per compound, no CI derivable")
    ...
    print("\n[Half-Life] Apparent half-life (2-point estimate, exploratory) vs. literature:")
    print("  NOTE: n=2 per compound -- values rounded (whole years, or months if <1y); no CI derivable.")
    print(f"  {'PFAS':<8} {'NHANES ~t½':>11}  {'Lit. t½':>8}  {'%diff':>7}  status")
    print(f"  {'-'*8}  {'-'*11}  {'-'*8}  {'-'*7}")
    for _, r in hl_df.iterrows():
        if r["status"] == "estimated":
            lit_str = _fmt_half_life(r['literature_half_life_years'])
            diff_str = f"{r['pct_diff_vs_literature']:+.0f}%" if pd.notna(r['pct_diff_vs_literature']) else "n/a"
            print(f"  {r['PFAS_Name']:<8} {'~' + _fmt_half_life(r['half_life_years']):>11}  {lit_str:>8}  {diff_str:>7}  estimated")
        else:
            print(f"  {r['PFAS_Name']:<8} {'—':>11}  {'—':>8}  {'—':>7}  {r['status']}")
    if not flagged.empty:
...
def main():
    ...
    if not hl_df.empty:
        print(f"\n  [Apparent Half-Life (2-point estimate) — exploratory, not a headline result]")
        estimated = hl_df[hl_df["status"] == "estimated"]
        print(f"    Compounds with a valid 2-wave estimate: {len(estimated)}  (n=2/compound, no CI derivable)")
        ...
        if not estimated.empty:
            ...
            if closest is not None:
                print(f"    Closest match to literature: {closest['PFAS_Name']} "
                      f"({closest['pct_diff_vs_literature']:+.0f}% vs. published value)")
