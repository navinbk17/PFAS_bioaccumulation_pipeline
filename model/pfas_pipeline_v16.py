"""
PFAS Bioaccumulation Data Pipeline v16.0
=========================================
...
def print_two_comp_summary(df_2comp, observed_bcf_df, df_1comp=None):
    ...
    for label, lst in [("IMPROVED", improved), ("DEGRADED", degraded), ("≈unchanged", unchanged)]:
        if lst:
            print(f"  {label}: " + ", ".join(
    ...
    if improved and not degraded and all(abs(e) < 25 for _,_,e,_ in improved+unchanged):
        print(f"\n  OUTCOME: Two-compartment RESOLVES the sulfonate triad.")
    elif improved and not degraded:
        print(f"\n  OUTCOME: Two-compartment IMPROVES sulfonates; residual error remains.")
        print(f"  Run tissue albumin sweep to identify calibration target.")
    elif degraded:
        print(f"\n  OUTCOME: Two-compartment DEGRADES sulfonates vs v14.1.")
        print(f"  Run tissue albumin sweep — Ktissue may need calibration.")
...
def main():
    print("\n" + "═" * 60)
    print("  PFAS Bioaccumulation Pipeline v16.0")
    print("  v16.0: LOCO CV as headline R² + relative paths")
    ...
    if len(ag_with_data) > 0:
        ...
        for _, r in ag_with_data.iterrows():
            ...
            if pd.notna(delta_13):
                if abs(pct_v14) < abs(pct_v13) and abs(pct_v14) < 20:
                    verdict = "RESOLVED"
                elif abs(pct_v14) < abs(pct_v13):
                    verdict = "IMPROVED"
