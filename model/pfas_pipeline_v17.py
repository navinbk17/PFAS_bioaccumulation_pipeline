"""
PFAS Bioaccumulation Data Pipeline v17.0
=========================================
Sources: ECOTOX + EPA CompTox + CDC NHANES (2015-2016, 2017-2018)

Changes in v17.0 — Target variable fix; BCF fish-only; per-PFAS leakage fix; multi-seed R²
--------------------------------------------------------------------------------------------
1. TARGET VARIABLE FIX (highest priority — changes conclusions)
   UNIT_TO_NG_G replaced with three typed dictionaries:
     TISSUE_UNITS_TO_NG_G  → Concentration_ng_g (ML target; mass/mass tissue only)
     MEDIUM_UNITS_TO_NG_L  → Medium_Concentration_ng_L (water; BCF denominator)
     DOSE_UNITS / SOIL_UNITS → excluded with counts reported
   harmonize() now routes each row to the correct column based on unit type.
   Dose records (mg/kg diet, ai mg/kg bdwt) and soil records are no longer
   mixed into the accumulation target. Dataset will shrink from ~25k to a
   defensible tissue-residue subset. Findings 3, 5, 8 should be re-evaluated.

2. BCF COMPARISON RESTRICTED TO FISH ONLY
   observed_bcf filter changed from isin(["Fish","Other"]) to == "Fish".
   "Other" is a catch-all for unmatched taxa (invertebrates, algae, etc.).
   PFHxS had zero fish records (Finding 6); its prior "observed BCF" came
   entirely from Other, making the fish physiology model comparison circular.
   n and IQR reported per compound so sparsity is explicit.
   Applied consistently in main(), Finding 18, and Finding 19 blocks.

3. PER-PFAS MODEL LEAKAGE FIXED
   Species group dummies removed from run_per_pfas_models(). Within a single
   compound all CONC_FEATURES are constant, so dummies were the only varying
   predictor. R²=0.657 for PFOA measured between-species-group variance —
   the exact leakage FIX 1 eliminated from the global model. Without dummies,
   per-compound models have no varying predictors; that is the finding.

4. MULTI-SEED R² REPLACES SINGLE-SPLIT
   evaluate_rf_multiseed() runs 20 seeds and reports mean ± SD.
   Called before the main single-seed split so the headline number carries
   proper uncertainty. Effective N reminder printed (# distinct compounds,
   not # rows).

5. NHANES TRANSPARENCY (expanded)
   Non-detect count reported at load time with upward-bias warning.
   Survey weights applied if WTSB2YR/WTSA2YR present: weighted mean
   computed per PFAS and compared to unweighted mean so bias is visible.
   Absence of weight column flagged with explicit representativeness warning.
   Units clarified: NHANES ng/mL ≈ ng/g assuming serum density ≈ 1 g/mL.
   Full non-detect imputation (LOD/√2) still requires LOD column — next step.

6. HALF-LIFE FRAMING CORRECTED
   plot_half_life_comparison() now prints explicit cross-sectional caveats
   before the table: two survey cycles give zero residual df; result confounds
   elimination with exposure trend; not a clinical elimination rate.

7. TWO-COMP CIRCULARITY ACKNOWLEDGED
   print_two_comp_summary() Finding 20 verdict block now flags that
   NLOM_TISSUE_FACTOR was selected to minimise PFOS error — residuals
   are in-sample fits, not independent validation. Over-parameterisation
   explicitly noted (PFHxS ±34–128% range).

Changes in v16.0 — LOCO validation as headline R²; relative paths; README refresh
-----------------------------------------------------------------------------------
1. Leave-One-Compound-Out (LOCO) validation added as the primary evaluation metric.
   run_loco_validation() holds out all rows for one PFAS at a time and trains on the
   rest — the correct test of "can chemistry predict accumulation for an unseen compound?"
   The pooled random-split R² (previously the headline number) is retained for
   comparison but is now clearly labelled as inflated: CONC_FEATURES are identical
   for every row of the same chemical, so random train/test splits put near-duplicate
   feature vectors in both sets. The pooled number mostly reflects between-compound
   variance (8 distinct patterns), not structure-activity learning.
   New output: loco_r2.png — per-compound LOCO R² bar chart.

2. Absolute hardcoded paths replaced with a relative-path config block at the top
   of the file. Data files are expected in data/ relative to the script directory;
   outputs go to outputs/. Both can be overridden by editing the config block.
   The outputs/ directory is created automatically if it does not exist.

3. README updated to v16.0 state: model results section reflects LOCO as headline,
   setup section documents the data/ layout, version history entry added.

Changes in v14.0 — Protein-facilitated gill uptake (k1_prot) — Option A
-------------------------------------------------------------------------
Motivation: Finding 18 (v13.0) established that tissue partitioning (Kfish)
is no longer the dominant source of sulfonate BCF error. Replacing the legacy
Koc-proxy with a direct albumin-Ka term for Kfish is mechanistically correct
but does not close the sulfonate gap. The root cause is the passive-diffusion
k1 formulation: Arnot-Gobas Eq. 5 computes k1 from Kow and a two-resistance
membrane term calibrated on neutral organics. For sulfonates (pKa ≈ -3.3),
the dominant uptake pathway is protein-facilitated transport — the albumin-
concentration gradient in fish blood pulls PFAS across the gill epithelium
faster than passive diffusion predicts.

Option A implementation (protein-facilitated k1 augmentation):
  The augmented k1 adds a facilitated-transport term alongside passive
  diffusion (Conder et al. 2008, Environ. Sci. Technol. 42:9283, Eq. 2;
  Ng & Hungerbühler 2013, Environ. Sci. Technol. 47:7214, Eq. 3):

    k1_passive = Ew × GV × mem_resistance          (Arnot-Gobas baseline)
    k1_fac     = K_FAC × Ka_albumin [L/mol]
                         × C_ALBUMIN_FISH_MOL_L [mol/L]
                         × GV [L/kg/d]
    k1         = k1_passive + k1_fac

  Interpretation:
    k1_fac represents the gill uptake flux driven by protein binding in
    fish blood. The thermodynamic gradient created by high Ka albumin
    binding sweeps dissolved PFAS across the gill epithelium; K_FAC is
    the fractional transfer efficiency of the protein-bound flux through
    the gill membrane per unit ventilation.

    K_FAC (dimensionless, 0 < K_FAC ≤ 1):
      = 1.0 → 100% of the protein-bound fraction crosses per ventilation
               volume (upper bound — perfectly efficient facilitated
               transport). Likely too high; treated as ceiling.
      = 0.01 → 1% of protein-bound fraction crosses. This is the
               empirically grounded starting point from Ng &
               Hungerbühler (2013) who derive k_fac ≈ 0.005–0.02 for
               PFOS and PFHxS from rainbow trout BCF data.
      Values: swept across K_FAC_SWEEP = [0.0, 0.001, 0.005, 0.01,
              0.02, 0.05, 0.10, 0.20, 0.50, 1.0] in
              run_arnot_gobas_kfac_sensitivity() to map full response
              surface before committing to K_FAC_DEFAULT.

    K_FAC_DEFAULT = 0.01 (literature-informed starting value; Ng &
                    Hungerbühler 2013). Can be overridden via constant.

  Why K_FAC is not compound-class-specific (unlike KPROT_SCALE):
    The facilitated-transport mechanism is expected to apply to any
    strongly protein-binding PFAS; the compound specificity enters
    through Ka_albumin, which is already measured per compound.
    K_FAC is the membrane transfer efficiency, which is a gill
    physiology parameter rather than a chemical property. Setting
    one K_FAC across all compounds keeps the model parsimonious and
    testable with current data. If carboxylates show systematic
    deviation, a class-specific dict can be introduced as a v14.1
    extension.

  Carboxylates (PFOA, PFNA, PFDA, etc.):
    The facilitated term is also added for carboxylates (Ka_albumin
    measured). Because carboxylates already fit well (v13.0: PFOA +5%,
    PFDA +2%), the K_FAC term should not substantially degrade them
    — but this is explicitly tested in the k_fac sensitivity sweep
    and reported per compound.

Finding 19 (evaluated at runtime — see main() summary):
  The k_fac sensitivity sweep identifies the value(s) of K_FAC that
  minimise the sulfonate BCF error. Outcomes:
    (a) Gap closes at K_FAC in the empirically plausible range (0.005–
        0.05): Finding 19 = protein-facilitated uptake quantitatively
        explains the sulfonate BCF gap. Paper narrative: complete arc
        Findings 14–19.
    (b) Gap partially closes, best K_FAC is ≫0.05: facilitated
        transport is directionally correct but single-compartment
        framework limits accuracy; Option B (two-compartment) is the
        recommended next step. This is still a publication-worthy
        finding — it quantifies how much of the gap is k1 vs
        structural model limitation.
    (c) Gap does not close at any K_FAC: the remaining error is not
        in k1 of any single-compartment form. This rules out Option A
        and mandates Option B; documented as a negative result that
        constrains the mechanistic space.

  All three outcomes are informative for the EST/EST Letters manuscript.

New/changed outputs (v14.0):
  arnot_gobas_bcf.png            — updated with v14 k1_prot BCF
  arnot_gobas_kfac_sensitivity.png — NEW: heatmap % error × K_FAC
  k1_passive and k1_fac columns  — added to compute_arnot_gobas_bcf() output
  k_fac column                   — facilitating efficiency per row
  Console Finding 19 block       — sulfonate gap vs v13.0 and v12.x baselines

Preserved from v12.x–v13.0 (historical diagnostics, unchanged):
  run_arnot_gobas_sensitivity()      — Kprot sweep; Finding 16 evidence
  run_arnot_gobas_pmem_sensitivity() — P_mem sweep; Finding 17 evidence
  compute_arnot_gobas_bcf() Kfish    — Ka_albumin formulation retained (v13.0)
...
# ── PATH CONFIGURATION ──────────────────────────────────────────────────────
# Edit these four paths to match your setup. All other paths in the script
# are derived from these — nothing else needs changing.
#
# Default: data files sit next to this script; outputs go to an outputs/
# subfolder alongside it (created automatically if it doesn't exist).
#
import os as _os
_BASE = _os.path.dirname(_os.path.abspath(__file__))  # folder containing this script

ECOTOX_EXPORT_DIR = _os.path.join(_BASE, "ecotox_exports") + _os.sep
COMPTOX_SNAPSHOT  = _os.path.join(_BASE, "comptox_snapshot.csv")
NHANES_PATH       = _os.path.join(_BASE, "nhanes_pfas_processed.csv")
OUTPUT_DIR        = _os.path.join(_BASE, "outputs") + _os.sep
...
# v14.0: Protein-facilitated gill uptake efficiency constant.
#
# Motivation: Finding 18 (v13.0) showed that correcting Kfish alone (via
# Ka_albumin tissue partition) does not close the sulfonate BCF gap. The
# remaining error is in k1 — the Arnot-Gobas passive gill diffusion term
# systematically under-predicts uptake for strongly protein-binding PFAS.
#
# The facilitated-transport k1 formulation augments passive diffusion with
# a protein-mediated flux term (Conder et al. 2008, Environ. Sci. Technol.
# 42:9283, Eq. 2; Ng & Hungerbühler 2013, Environ. Sci. Technol. 47:7214,
# Eq. 3):
#
#   k1_passive = Ew × GV × mem_resistance     (Arnot-Gobas baseline, unchanged)
#   k1_fac     = K_FAC × Ka_albumin [L/mol]
#                        × C_ALBUMIN_FISH_MOL_L [mol/L]
#                        × GV [L/kg/d]
#   k1         = k1_passive + k1_fac
#
# K_FAC (dimensionless): fractional gill membrane transfer efficiency for the
# protein-bound fraction of PFAS per unit ventilation volume. Represents the
# probability that a PFAS molecule bound to albumin at the gill surface crosses
# the membrane into blood per unit ventilation volume.
#
# Literature range:
#   Ng & Hungerbühler (2013): back-calculate k_fac ≈ 0.005–0.02 from rainbow
#     trout BCF data for PFOS and PFHxS using a two-resistance + facilitated-
#     transport framework. These are the best-constrained empirical values.
#   Conder et al. (2008): propose the conceptual framework; do not quantify k_fac
#     directly but suggest facilitated transport is necessary to explain PFOS
#     BCF in fish.
#   Upper bound (K_FAC = 1.0): 100% efficient facilitated transport — all
#     protein-bound PFAS at the gill surface crosses per ventilation event.
#     Physically unrealistic but useful as a ceiling to confirm the sweep
#     bounds the observable range.
#   K_FAC = 0.0: pure passive diffusion — reverts to v13.0 behaviour exactly.
#     Used as the sweep baseline to confirm continuity with prior versions.
#
# K_FAC_DEFAULT = 0.01: Ng & Hungerbühler (2013) midpoint; used in the main
# compute_arnot_gobas_bcf() call. The sweep maps sensitivity around this value.
# If the sweep reveals a sharper minimum, K_FAC_DEFAULT should be updated.
# v14.0 (class-agnostic): K_FAC_DEFAULT = 0.01 applied to ALL PFAS.
# v14.1 finding: carboxylates that already fit at K_FAC=0 (PFOA +5%, PFDA +2%)
# blew to +636%/+154% the moment any facilitated term was added. The additive
# formulation cannot simultaneously fit sulfonates and carboxylates with a
# shared scalar because carboxylates' passive k1 already explains their BCF;
# adding a facilitated term overcounts their uptake.
#
# v14.1 fix: class-conditional K_FAC.
#   Sulfonate:   K_FAC = K_FAC_DEFAULT (swept below) — facilitated transport required
#   Carboxylate: K_FAC = 0.0 (hard-coded zero) — passive diffusion sufficient
#
# Mechanistic justification: sulfonates (pKa ≈ -3.3) carry full negative charge
# at physiological pH and cannot cross phospholipid bilayers by passive diffusion
# at the rate their observed BCF implies. Carboxylates (pKa ≈ 0-1) at pH 7.4
# are ~99.97% ionized but have significantly lower charge density and higher Kow
# at equivalent chain length — their passive k1 (via Arnot-Gobas Eq. 5) already
# accounts for their observed BCF without a facilitated term.
K_FAC_DEFAULT = 0.01   # applied to sulfonates only (v14.1); carboxylates fixed at 0.0

# Sweep values for run_arnot_gobas_kfac_sensitivity() (v14.1: sulfonate-only).
# K_FAC=0.0 entry reproduces v13.0 (pure passive, no facilitated term).
# Tighter resolution around the Ng & Hungerbühler (2013) empirical range (0.005–0.02).
K_FAC_SWEEP = [0.0, 0.001, 0.005, 0.01, 0.02, 0.05, 0.10, 0.20, 0.50, 1.0]

# ── v15.0: Two-compartment model constants (Option B) ────────────────────────
#
# Motivation: Finding 19 confirmed the single-compartment framework is
# structurally closed. The two-compartment model separates:
#   Compartment 1 — blood/plasma (V1=0.05): high-affinity albumin pool
#                   [albumin]_blood = 20 g/L (Conder et al. 2008)
#   Compartment 2 — peripheral tissue (V2=0.95): lower-affinity bulk pool
#                   [albumin]_tissue = 5 g/L (Barber 2003, interstitial fluid)
# This separation allows Kblood and Ktissue to differ, resolving the
# structural conflict identified in Finding 19 where a shared Kfish
# conflated both pools.
#
# References:
#   Barber (2003) Chemosphere 53:1099 — interstitial [albumin] ≈ 5 g/L
#   Farrell (1991) J. Exp. Biol. 159:213 — cardiac output teleosts
#   Nichols et al. (2004) Environ. Toxicol. Chem. 23:2017 — 2-comp fish TK

V1 = 0.05          # blood compartment volume fraction (Barber 2003)
V2 = 1.0 - V1     # tissue compartment volume fraction

Q_BLOOD = 1.0      # cardiac output (L blood / kg fish / d) — Farrell (1991)
                   # resting teleost at 12°C; range 0.5–2.0 L/kg/d
Q_BLOOD_SWEEP = [0.25, 0.50, 1.0, 2.0, 4.0]

C_ALBUMIN_BLOOD_G_L  = 20.0   # plasma albumin (g/L) — Conder et al. (2008)
C_ALBUMIN_TISSUE_G_L =  5.0   # interstitial albumin (g/L) — Barber (2003)
MW_ALBUMIN_2COMP     = 68000.0  # fish serum albumin MW — Chen et al. (2016)

C_ALBUMIN_BLOOD_MOL_L  = C_ALBUMIN_BLOOD_G_L  / MW_ALBUMIN_2COMP  # ≈ 2.94e-4
C_ALBUMIN_TISSUE_MOL_L = C_ALBUMIN_TISSUE_G_L / MW_ALBUMIN_2COMP  # ≈ 7.35e-5

# Tissue composition — peripheral compartment (Arnot & Gobas 2004, Table 1)
VL_TISSUE  = 0.05   # lipid fraction (excluded from Ktissue — see compute_two_comp_bcf)
VN_TISSUE  = 0.09   # NLOM fraction
VW_TISSUE  = 0.72   # water fraction
VW_BLOOD   = 0.94   # plasma water fraction

# Tissue albumin sweep — retired after v15.0 run showed invariance.
# Increasing C_alb_tissue 5→80 g/L changed PFOS error by <7pp because
# the NLOM term (0.035 × Koc = 73.5 for PFOS) dominates Ktissue and
# swamps the albumin contribution entirely.
C_ALB_TISSUE_SWEEP = [5.0, 10.0, 20.0, 40.0, 80.0]  # g/L (archived)

# NLOM tissue factor sweep — Finding 20 primary calibration parameter.
#
# In the single-compartment model (v11–v14), Kd_nlom = 0.035 × Koc was
# used for the whole-body fish partition coefficient (Kfish). The 0.035
# coefficient was calibrated on SEDIMENT organic carbon sorption (Gobas
# et al. 2003) — a fundamentally different matrix from fish muscle tissue.
#
# In the two-compartment model, Ktissue represents bulk peripheral tissue
# (muscle, organ), not sediment. PFAS tissue-NLOM sorption in fish muscle
# is expected to be substantially weaker than sediment sorption because:
#   (a) Fish tissue NLOM is largely structural protein and phospholipid,
#       not diagenetically condensed humic material that drives high Koc.
#   (b) PFAS in tissue bind predominantly to albumin/protein, not to
#       organic carbon fractions — the NLOM pathway is a secondary
#       mechanism at best.
#
# The v15.0 tissue albumin sweep showed BCF is invariant to C_alb_tissue
# (5→80 g/L causes <7pp change for PFOS) because Kd_nlom = 73.5 >> any
# albumin term. Sweeping NLOM_TISSUE_FACTOR tests whether reducing the
# NLOM coefficient to a tissue-appropriate value brings BCF into range.
#
# Literature support for reduced NLOM in fish tissue:
#   Kelly et al. (2004) Environ. Sci. Technol. — tissue NLOM partition
#     for PFAS is ~5–10× lower than sediment Koc-normalized value.
#   Conder et al. (2008) — whole-body tissue Kd estimated without a
#     large NLOM contribution for PFAS; protein binding dominates.
#
# Sweep range: 0.035 (Gobas sediment baseline) down to 0.0 (protein-only).
# The zero-crossing for the sulfonate triad identifies the tissue-NLOM
# coefficient consistent with observed fish BCF data.
# Class-specific NLOM tissue factors (Finding 20 calibration).
#
# A single global nlom_factor cannot simultaneously fit sulfonates
# (need low NLOM — tissue sorption is protein-dominated, Ka >> 10^3.5)
# and carboxylates (need higher NLOM — lower Ka means organic-carbon
# sorption is a larger fraction of tissue partitioning).
#
# Calibration basis:
#   Sulfonates:   nlom_factor=0.002 — PFOS zero-crossing from NLOM sweep.
#                 Kelly et al. (2004): tissue NLOM ~17× below sediment Koc.
#   Carboxylates: nlom_factor=0.010 — retains meaningful NLOM contribution
#                 for low-Ka compounds (PFOA Ka=10^3.65, PFNA Ka=10^4.10)
#                 where tissue protein binding is weaker and organic-carbon
#                 sorption is a proportionally larger fraction of Ktissue.
#                 Value is ~3.5× below sediment baseline (0.035), consistent
#                 with the Kelly et al. (2004) range for aquatic organisms.
NLOM_TISSUE_FACTOR = {
    "Sulfonate":   0.002,
    "Carboxylate": 0.010,
}
NLOM_TISSUE_FACTOR_SWEEP = [0.035, 0.010, 0.005, 0.002, 0.001, 0.0005, 0.0001, 0.0]
...
ECOTOX_COL_ALIASES = {
    ...
    "DOI":               ["DOI"],
    "Effect":            ["Effect", "Observed Effect", "Biological Effect"],
    "Endpoint":          ["Endpoint", "Statistical Method", "Endpoint Code"],
    "Measurement":       ["Measurement", "Observed Response Measurement", "Response Measurement"],
}

# v17.0: unit map split into three typed dictionaries.
# Previously all units fed a single Concentration_ng_g column, mixing tissue
# residues, water concentrations, soil concentrations, and administered doses —
# four incommensurable quantities. This is the primary cause of model noise
# (mentor feedback, priority 1).
#
# TISSUE_UNITS_TO_NG_G  → Concentration_ng_g  (the ML target; mass/mass tissue)
# MEDIUM_UNITS_TO_NG_L  → Medium_Concentration_ng_L (water/media; BCF denominator)
# DOSE_UNITS            → flagged and excluded (study design parameters, not outcomes)
# SOIL_UNITS            → excluded (not bioaccumulation endpoints)

TISSUE_UNITS_TO_NG_G = {
    # Mass per mass of tissue — the only defensible target for bioaccumulation
    "ng/g":     1,
    "ug/g":     1_000,
    "µg/g":     1_000,
    "mg/kg":    1_000,       # mg/kg wet wt ≡ ug/g ≡ ppb
    "mg/g":     1_000_000,
    "pg/g":     0.001,
    "ppb dry wt": 1,         # ppb = ng/g
    "ppm":      1_000,       # ppm = ug/g = 1000 ng/g
    "ug/g egg": 1_000,       # egg tissue
    "mg/kg egg": 1_000,      # egg tissue (alt unit)
    "ng/g dw":  1,           # dry-weight tissue (not soil)
}

MEDIUM_UNITS_TO_NG_L = {
    # Water / exposure medium concentrations — ng/L scale
    # Kept separate; used as BCF denominator, NOT as ML target
    "ng/l":   1,
    "ug/l":   1_000,
    "µg/l":   1_000,
    "mg/l":   1_000_000,
    "ug/ml":  1_000,         # ug/mL = mg/L = 1000 ug/L
    "ai mg/l": 1_000_000,    # active ingredient mg/L
}

# Dose units — administered doses, not tissue residues.
# These are study design parameters and must not appear in the accumulation target.
DOSE_UNITS = {
    "ai mg/kg bdwt", "ai mg/kg food", "mg/kg diet",
    "mg/kg bw/d", "ug/kg bw/d", "mg/kg bw",
}

# Soil/sediment units — not bioaccumulation endpoints for aquatic/mammal models.
SOIL_UNITS = {
    "mg/kg soil", "mg/g soil", "ng/g soil", "ug/kg soil",
    "mg/kg dry soil", "ug/kg dry soil", "ng/g dw soil",
}

# v17.0: ECOTOX endpoint/measurement filter.
# Only keep records where the measurement type is a tissue residue or
# accumulation ratio. Water concentration records are retained but routed to
# Medium_Concentration_ng_L; all other endpoint types are excluded.
#
# ECOTOX Endpoint column codes for accumulation (non-exhaustive; cover the
# vast majority of bioaccumulation studies):
ACCUMULATION_ENDPOINTS = {
    "BCF", "BAF", "BSAF", "BMF", "TMF",                 # ratio endpoints
    "BCF_SS", "BAF_SS",                                  # steady-state variants
}
# ECOTOX Measurement column substrings indicating tissue residue
RESIDUE_MEASUREMENT_KEYWORDS = {
    "residue", "accumulation", "concentration", "tissue", "body burden",
    "whole body", "muscle", "liver", "blood", "serum", "plasma",
    "egg", "gonad", "fat", "blubber", "kidney", "brain",
}
...
def load_ecotox(export_dir):
    ...
    extra_cols = ["BCF 2 Value", "BCF 3 Value"]
    keep = [c for c in ECOTOX_COL_ALIASES.keys() if c in combined.columns] + \
       [c for c in extra_cols if c in combined.columns]
    return combined[keep].copy()


def harmonize(df):
    """
    v17.0: Three-stage unit routing replaces monolithic UNIT_TO_NG_G.

    Stage 1 — Unit classification per row:
      tissue unit  → Concentration_ng_g   (ML target)
      medium unit  → Medium_Concentration_ng_L  (BCF denominator; not target)
      dose unit    → flagged as Unit_Type='dose', excluded from ML target
      soil unit    → flagged as Unit_Type='soil', excluded from ML target
      unknown unit → Unit_Type='unknown', excluded from ML target

    Stage 2 — Endpoint filter:
      Rows are retained if they have a tissue Concentration_ng_g OR if their
      unit is a medium unit (water exposure records, useful for BCF calc).
      Rows whose only result is a dose or soil unit are dropped.

    Stage 3 — log_concentration computed on tissue rows only.
      Medium rows keep Medium_Concentration_ng_L but have NaN log_concentration
      so they do not enter any ML target training.

    Non-detects: rows where Result Value == 0 are flagged as Non_Detect=True
      and imputed as LOD/sqrt(2) where LOD can be inferred; otherwise they
      retain Concentration_ng_g=NaN and are excluded. Count is reported.
    """
    df = df.copy()
    ...
    def classify_unit(unit_str):
        ...
        if u in TISSUE_UNITS_TO_NG_G:
            return "tissue"
        if u in MEDIUM_UNITS_TO_NG_L:
            return "medium"
        if u in DOSE_UNITS:
            return "dose"
        if u in SOIL_UNITS:
    ...
    df["Unit_Type"] = df["Result Unit"].apply(classify_unit)

    # ── Endpoint filter ───────────────────────────────────────────────
    # Keep a row if ANY of these is true:
    #   (a) Effect column contains "accumulation" (case-insensitive)
    #   (b) Endpoint column is a known accumulation code (BCF, BAF, etc.)
    #   (c) Measurement column contains a residue keyword
    #   (d) Row has a non-null BCF column value (explicit BCF field)
    #   (e) Unit is a tissue unit (already caught — tissue residue regardless of endpoint)
    #   (f) Unit is a medium unit — keep for BCF denominator use
    #
    # Rows that are purely toxicity endpoints (LC50, NOEC, growth) with
    # water/dose units are excluded here.
    def is_accumulation_row(row):
        # Always keep tissue-unit rows — they're residues by definition
        if row.get("Unit_Type") == "tissue":
            return True
        # Always keep rows with an explicit BCF value
        if pd.notna(row.get("BCF")):
            return True
        # Check Effect column
        effect = str(row.get("Effect", "")).lower()
        if "accumulation" in effect or "residue" in effect or "bcf" in effect or "baf" in effect:
            return True
        # Check Endpoint column
        endpoint = str(row.get("Endpoint", "")).strip().upper()
        if endpoint in ACCUMULATION_ENDPOINTS:
            return True
        # Check Measurement column
        measurement = str(row.get("Measurement", "")).lower()
        if any(kw in measurement for kw in RESIDUE_MEASUREMENT_KEYWORDS):
            return True
        # Medium-unit rows: only keep if they have an accumulation endpoint context
        # (water conc in a toxicity test is not useful; water conc in a BCF study is)
        if row.get("Unit_Type") == "medium":
            if any([
                "accumulation" in effect, "residue" in effect,
                endpoint in ACCUMULATION_ENDPOINTS,
                any(kw in measurement for kw in RESIDUE_MEASUREMENT_KEYWORDS),
            ]):
                return True
        return False

    n_before_ep = len(df)
    df["is_accumulation"] = df.apply(is_accumulation_row, axis=1)
    n_excluded_ep = (~df["is_accumulation"]).sum()
    df = df[df["is_accumulation"]].drop(columns=["is_accumulation"])
    print(f"[Harmonize] Endpoint filter: excluded {n_excluded_ep:,} non-accumulation rows "
          f"({n_before_ep:,} → {len(df):,})")

    # Count what we're dropping before filter
    n_total_raw = len(df)
    n_dose  = (df["Unit_Type"] == "dose").sum()
    n_soil  = (df["Unit_Type"] == "soil").sum()
    n_unk   = (df["Unit_Type"] == "unknown").sum()
    n_tissue = (df["Unit_Type"] == "tissue").sum()
    n_medium = (df["Unit_Type"] == "medium").sum()
    print(f"[Harmonize] Unit classification: tissue={n_tissue:,}  medium={n_medium:,}  "
          f"dose={n_dose:,}  soil={n_soil:,}  unknown={n_unk:,}  total={n_total_raw:,}")
    if n_dose > 0:
        print(f"[Harmonize] EXCLUDED {n_dose:,} dose rows (study design parameters, not tissue residues)")
    if n_soil > 0:
        print(f"[Harmonize] EXCLUDED {n_soil:,} soil/sediment rows (not bioaccumulation endpoints)")

    # ── Tissue concentration ──────────────────────────────────────────
    def tissue_conc(row):
        if row["Unit_Type"] != "tissue":
            return np.nan
        unit = str(row.get("Result Unit", "")).strip().lower()
        f = TISSUE_UNITS_TO_NG_G.get(unit)
        val = row["Result Value"]
        return val * f if (f and pd.notna(val)) else np.nan

    df["Concentration_ng_g"] = df.apply(tissue_conc, axis=1)

    # ── Medium concentration ──────────────────────────────────────────
    def medium_conc(row):
        if row["Unit_Type"] != "medium":
            return np.nan
        unit = str(row.get("Result Unit", "")).strip().lower()
        f = MEDIUM_UNITS_TO_NG_L.get(unit)
        val = row["Result Value"]
        return val * f if (f and pd.notna(val)) else np.nan

    df["Medium_Concentration_ng_L"] = df.apply(medium_conc, axis=1)

    # ── Non-detect flagging ───────────────────────────────────────────
    # Zero values in tissue rows = non-detect (below LOD). NHANES uses LOD/√2
    # imputation; for ECOTOX we flag and drop (no LOD column available).
    n_nondetect = ((df["Unit_Type"] == "tissue") & (df["Result Value"] == 0)).sum()
    if n_nondetect > 0:
        print(f"[Harmonize] Non-detects (tissue, value=0): {n_nondetect:,} rows excluded "
              f"(no LOD column in ECOTOX export; count for reporting)")
    df.loc[(df["Unit_Type"] == "tissue") & (df["Result Value"] == 0), "Concentration_ng_g"] = np.nan

    # ── Keep only rows with a usable tissue OR medium concentration ───
    has_tissue = df["Concentration_ng_g"].notna() & (df["Concentration_ng_g"] > 0)
    has_medium = df["Medium_Concentration_ng_L"].notna() & (df["Medium_Concentration_ng_L"] > 0)
    df = df[has_tissue | has_medium].copy()
    print(f"[Harmonize] After unit filter: {len(df):,} rows "
          f"(tissue={has_tissue.sum():,}  medium={has_medium.sum():,})")

    # ── log_concentration — tissue rows only ─────────────────────────
    df["log_concentration"] = df["Concentration_ng_g"].apply(
...
def evaluate_rf_multiseed(X, y, groups, n_seeds=20, test_size=0.2, label=""):
    """
    v17.0: Replace single-split R² with mean ± SD across n_seeds random splits.

    Mentor feedback: single random_state=42 R² values quoted to 3 decimal
    places attach false precision. With only 8 distinct chemistry profiles,
    split-to-split variance is large and must be reported.

    Returns dict with mean_r2, std_r2, mean_rmse, std_rmse, and per-seed records.
    Stratifies by groups (Species_Group) to keep group proportions stable.
    """
    records = []
    ...
    print(f"[MultiSeed{tag}] R² = {result['mean_r2']:.3f} ± {result['std_r2']:.3f}  "
          f"RMSE = {result['mean_rmse']:.3f} ± {result['std_rmse']:.3f}  "
          f"(n_seeds={n_seeds}, test_size={test_size})")
    print(f"[MultiSeed{tag}] NOTE: confidence appropriate to "
          f"{len(set(groups.values if hasattr(groups,'values') else groups))} "
          f"distinct chemistry profiles, not {len(y):,} rows")
    return result
...
def plot_half_life_comparison(hl_df, path):
    ...
    print(f"  NOTE: These are CROSS-SECTIONAL estimates from 2 NHANES waves, NOT")
    print(f"  individual elimination rates. They conflate true elimination with")
    print(f"  changes in population exposure, cohort composition, and age structure.")
    print(f"  Two time points give zero residual degrees of freedom — no uncertainty")
    print(f"  interval is derivable. Compare to literature half-lives as a sanity")
    print(f"  check only; do NOT interpret as clinical or regulatory elimination rates.")
...
def run_per_pfas_models(dataset, avail_conc, global_rf, X_test_global, y_test_global,
    ...
    for pfas_name in all_pfas_names:
        ...
        # v17.0: species group dummies REMOVED.
        # Within a single compound every chemistry feature (avail_conc) is
        # constant, so species dummies were the only varying predictor. The
        # resulting R² measured between-species-group variance — the exact
        # leakage FIX 1 removed from the global model. Without dummies,
        # per-compound models have zero varying predictors; that is the honest
        # finding: chemistry features alone cannot predict within-compound
        # variance because they are constant within a compound.
...
def run_loco_validation(dataset, output_dir):
    """
    Leave-One-Compound-Out (LOCO) cross-validation for the concentration model.

    This is the correct headline evaluation of "can chemistry predict
    bioaccumulation?" — it answers: given the chemistry features of a compound
    the model has never seen, how well can it predict that compound's
    tissue concentrations?

    Standard random-split R² is misleading here because:
      (a) CONC_FEATURES are identical for every row of the same PFAS, so
          train and test share input patterns whenever rows from the same
          compound appear in both sets.
      (b) ~94% of rows are NHANES human data, so pooled R² mostly reflects
          between-compound variance, not genuine structure-activity learning.

    LOCO removes both problems by ensuring the held-out compound's rows
    never appear in training. With only 8 distinct chemistry profiles,
    LOCO R² will be low — that is the honest answer, and the right one to
    lead with rather than the inflated pooled number.

    Mirrors the cross-species LeaveOneGroupOut already in the pipeline
    (cross_species_validation), but keyed on PFAS compound rather than
    species group.
    """
    print("\n" + "─" * 60)
    print("  [LOCO] Leave-One-Compound-Out CV — headline chemistry R²")
    print("─" * 60)
    print("  Each fold: hold out ALL rows for one PFAS; train on rest.")
    print("  This is the honest test of structure → accumulation prediction.")

    avail = [c for c in CONC_FEATURES if c in dataset.columns]
    core  = ["Chain_Length", "MW", "LogKow", "log_concentration", "PFAS_Name"]
    ml = dataset[avail + ["log_concentration", "PFAS_Name"]].dropna(subset=core)

    X      = ml[avail].values
    y      = ml["log_concentration"].values
    groups = ml["PFAS_Name"].values

    logo   = LeaveOneGroupOut()
    rf_loco = RandomForestRegressor(n_estimators=200, random_state=42, n_jobs=-1)

    compound_results = []
    all_true, all_pred = [], []

    for train_idx, test_idx in logo.split(X, y, groups):
        compound = groups[test_idx[0]]
        X_tr, X_te = X[train_idx], X[test_idx]
        y_tr, y_te = y[train_idx], y[test_idx]

        # Need at least 2 distinct chemistry profiles in training to be useful
        unique_train = len(set(groups[train_idx]))
        if unique_train < 2 or len(y_te) < 5:
            compound_results.append({
                "PFAS": compound, "n_test": len(y_te),
                "R2": np.nan, "RMSE": np.nan, "note": "too_few_train_compounds",
            })
            continue

        rf_loco.fit(X_tr, y_tr)
        y_pred = rf_loco.predict(X_te)

        r2   = r2_score(y_te, y_pred)
        rmse = mean_squared_error(y_te, y_pred) ** 0.5

        # Per-compound mean baseline (what the model must beat)
        y_base = np.full_like(y_te, y_tr.mean())
        r2_base = r2_score(y_te, y_base)

        compound_results.append({
            "PFAS": compound, "n_test": len(y_te),
            "R2": r2, "RMSE": rmse, "R2_mean_baseline": r2_base, "note": "ok",
        })
        all_true.extend(y_te)
        all_pred.extend(y_pred)

        print(f"  Held out {compound:<10}  n={len(y_te):>5}  "
              f"R²={r2:+.3f}  RMSE={rmse:.3f}  "
              f"(mean-baseline R²={r2_base:+.3f})")

    loco_df = pd.DataFrame(compound_results)

    # Overall pooled LOCO R² across all held-out predictions
    if all_true:
        r2_loco_pooled = r2_score(all_true, all_pred)
        rmse_loco_pooled = mean_squared_error(all_true, all_pred) ** 0.5
    else:
        r2_loco_pooled = np.nan
        rmse_loco_pooled = np.nan

    print(f"\n  ── LOCO Summary ──────────────────────────────────────")
    print(f"  Pooled LOCO R²  : {r2_loco_pooled:+.3f}")
    print(f"  Pooled LOCO RMSE: {rmse_loco_pooled:.3f}")
    print(f"  Interpretation  : R² above measures whether chemistry")
    print(f"                    features can predict an unseen compound's")
    print(f"                    accumulation — the scientifically meaningful")
    print(f"                    question. Compare to pooled random-split R²,")
    print(f"                    which conflates between-compound and")
    print(f"                    within-compound variance (see Laura's note).")
    print(f"  ──────────────────────────────────────────────────────")

    # Plot: per-compound LOCO R² bar chart
    ok_df = loco_df[loco_df["note"] == "ok"].sort_values("R2")
    if not ok_df.empty:
        fig, ax = plt.subplots(figsize=(8, max(4, len(ok_df) * 0.6)))
        colors = ["coral" if r < 0 else "steelblue" for r in ok_df["R2"]]
        bars = ax.barh(ok_df["PFAS"], ok_df["R2"], color=colors)
        ax.axvline(0, color="black", lw=0.8)
        ax.axvline(r2_loco_pooled, color="darkred", lw=1.5, ls="--",
                   label=f"Pooled LOCO R²={r2_loco_pooled:+.3f}")
        for bar, (_, row) in zip(bars, ok_df.iterrows()):
            x_off = 0.01 if row["R2"] >= 0 else -0.01
            ha    = "left" if row["R2"] >= 0 else "right"
            ax.text(row["R2"] + x_off, bar.get_y() + bar.get_height() / 2,
                    f"n={int(row['n_test'])}", va="center", ha=ha, fontsize=8)
        ax.set_xlabel("Held-out R² (compound never seen during training)")
        ax.set_title(
            "Leave-One-Compound-Out CV — Concentration Model\n"
            "Honest test: can chemistry predict an unseen PFAS's accumulation?"
        )
        ax.legend(fontsize=8)
        plt.tight_layout()
        plt.savefig(output_dir + "loco_r2.png", dpi=150)
        plt.close()
        print(f"\n  [Output] → {output_dir}loco_r2.png")

    print("─" * 60)
    return {
        "loco_df": loco_df,
        "r2_loco_pooled": r2_loco_pooled,
        "rmse_loco_pooled": rmse_loco_pooled,
    }
...
def run_human_only_model(dataset, output_dir):
    ...
    X_fit, X_cal, y_fit, y_cal, pf_fit, pf_cal = train_test_split(
        X_train, y_train, pf_train, test_size=0.2, random_state=42,
        stratify=pf_train)
...
def compute_arnot_gobas_bcf(pfas_features_df, k_fac=None):
    """
    ...
    """
    # v14.0: resolve k_fac — allows sensitivity sweep to override module default
    if k_fac is None:
        k_fac = K_FAC_DEFAULT
    ...
    for _, row in pfas_features_df.iterrows():
        ...
        #
        # v12.1: P_MEM_CORRECTION applied here (retained for diagnostic
        # continuity with v12.x). In practice P_MEM_CORRECTION=0.50 for
        # sulfonates was shown by Finding 17 to WORSEN the error; the v14.0
        # facilitated-transport term (k1_fac below) supersedes this as the
        # primary mechanism for protein-binding PFAS. P_MEM_CORRECTION is
        # retained at its v12.x value but its effect is now secondary to k1_fac.
        p_mem = P_MEM_CORRECTION.get(pfas_class, 1.0)
        mem_resistance = 1.0 / (5.49e7 * (kow ** -0.670) + 1.0)
        k1_raw     = Ew * GV * mem_resistance    # uncorrected Arnot-Gobas passive k1
        k1_passive = k1_raw * p_mem              # p_mem correction (v12.1, legacy)

        # v14.1: class-conditional protein-facilitated gill uptake term.
        # Sulfonates: k1_fac = K_FAC × Ka_albumin × [albumin]_blood × GV
        # Carboxylates: k1_fac = 0.0 (passive diffusion already explains BCF)
        #
        # Motivation: v14.0 applied a shared K_FAC to all PFAS. The sweep showed
        # that carboxylates (PFOA, PFDA) catastrophically over-predict at any
        # K_FAC > 0 because their passive k1 already fits their observed BCF.
        # Adding a facilitated term overcounts uptake for carboxylates while
        # being genuinely necessary for sulfonates. The class-conditional
        # formulation reflects the mechanistic reality: sulfonates at pH 7.4
        # cannot cross the gill epithelium at the rate passive diffusion predicts,
        # whereas carboxylates can.
        #
        # k1_fac is zero for:
        #   - All Carboxylates (class-conditional, regardless of Ka_albumin)
        #   - Emerging PFAS with NaN Ka_albumin (no measured binding constant)
        if pfas_class == "Sulfonate" and pd.notna(ka_log):
            ka_mol_L_k1 = 10 ** ka_log
            k1_fac = k_fac * ka_mol_L_k1 * C_ALBUMIN_FISH_MOL_L * GV
        else:
            k1_fac = 0.0   # Carboxylates: passive diffusion sufficient
        ...
        rows.append({
            ...
            "Kfish":        Kfish,
            "k1_raw":       k1_raw,      # v12.1: uncorrected Arnot-Gobas passive k1
            "p_mem":        p_mem,       # v12.1: membrane permeability correction
            "k1_passive":   k1_passive,  # v14.0: passive component (k1_raw × p_mem)
            "k1_fac":       k1_fac,      # v14.0: facilitated-transport component
            "k_fac":        k_fac,       # v14.0: efficiency constant used this call
            "k1":           k1,          # v14.0: total k1 = k1_passive + k1_fac
            "k2":           k2,
            "ke":           ke,
            "kd":           kd,
            "km":           km,
...
def run_arnot_gobas_sensitivity(pfas_features_df, observed_bcf_df,
    ...
    for scale in sweep_values:
        ...
        for _, row in tmp_features.iterrows():
            ...
            Kfish    = Kprot + Kd_nlom + Vw
            Kfood    = (Vl_food * koc + 0.035 * Vn_food * koc + Vw_food
                        if pd.notna(koc) and koc > 0
                        else Vl_food * kow + 0.035 * Vn_food * kow + Vw_food)
...
def run_arnot_gobas_kfac_sensitivity(pfas_features_df, observed_bcf_df,
                                     sweep_values=K_FAC_SWEEP, path=None):
    """
    v14.0: Sensitivity sweep of the protein-facilitated gill uptake efficiency
    constant (K_FAC) in the augmented k1 formulation.

    This is the Option A test: if protein-facilitated transport explains the
    sulfonate BCF gap (Finding 18), then there exists a physically plausible
    K_FAC (0.005–0.02 per Ng & Hungerbühler 2013) that closes it. The sweep
    maps the full response surface — % error per PFAS vs K_FAC — so the result
    is interpretable regardless of whether the gap closes, partially closes, or
    does not respond.

    Parallel structure to run_arnot_gobas_sensitivity() (Kprot sweep, Finding 16)
    and run_arnot_gobas_pmem_sensitivity() (P_mem sweep, Finding 17).
    All three sweeps together constitute the complete single-compartment parameter
    elimination series for the paper's mechanistic narrative.

    K_FAC = 0.0 in the sweep corresponds to pure passive diffusion (v13.0
    baseline), confirming numerical continuity with the prior version.

    Parameters
    ----------
    pfas_features_df : DataFrame — curated PFAS feature table (PFAS_FEATURES)
    observed_bcf_df  : DataFrame with columns PFAS_Name, log_BCF (ECOTOX records)
    sweep_values     : list of K_FAC values to test
    path             : output path for heatmap PNG; None = console only

    Returns
    -------
    DataFrame with columns: k_fac, PFAS_Name, PFAS_Class, log_BCF_AG, pct_err
    """
    obs_median = (observed_bcf_df.groupby("PFAS_Name")["log_BCF"]
                  .median().rename("obs_log_bcf"))

    records = []
    for kf in sweep_values:
        ag = compute_arnot_gobas_bcf(pfas_features_df, k_fac=kf)
        for _, row in ag.iterrows():
            obs = obs_median.get(row["PFAS_Name"], np.nan)
            pct_err = ((row["BCF_AG"] - 10**obs) / 10**obs * 100
                       if pd.notna(obs) and pd.notna(row["BCF_AG"]) else np.nan)
            records.append({
                "k_fac":      kf,
                "PFAS_Name":  row["PFAS_Name"],
                "PFAS_Class": row["PFAS_Class"],
                "log_BCF_AG": row["log_BCF_AG"],
                "k1_passive": row["k1_passive"],
                "k1_fac":     row["k1_fac"],
                "k1":         row["k1"],
                "pct_err":    pct_err,
            })
    ...
    # ── Console summary ───────────────────────────────────────────────
    print("\n[AG K_fac Sensitivity] Protein-facilitated gill uptake sweep (v14.1)")
    print(f"  Sulfonates: k1 = k1_passive + K_FAC × Ka_albumin × [albumin]_blood × GV")
    print(f"  Carboxylates: k1 = k1_passive (K_FAC=0 fixed — passive diffusion sufficient)")
    print(f"  Literature range: K_FAC ≈ 0.005–0.02 (Ng & Hungerbühler 2013)")
    print(f"  K_FAC=0.0 → pure passive diffusion for all PFAS (v13.0 baseline)\n")

    sulfonates   = sweep_df[sweep_df["PFAS_Class"] == "Sulfonate"]["PFAS_Name"].unique()
    carboxylates = sweep_df[sweep_df["PFAS_Class"] == "Carboxylate"]["PFAS_Name"].unique()

    # Only show PFAS with observed BCF data in the header
    sulfs_obs  = [p for p in sulfonates   if obs_median.get(p) is not np.nan and pd.notna(obs_median.get(p, np.nan))]
    carbs_obs  = [p for p in carboxylates if obs_median.get(p) is not np.nan and pd.notna(obs_median.get(p, np.nan))]

    header = f"  {'K_FAC':>8} | " + " ".join(f"{p:>8}" for p in sulfs_obs) + \
             "  |  " + " ".join(f"{p:>8}" for p in carbs_obs)
    print(header)
    print("  " + "-" * (len(header) - 2))

    for kf in sweep_values:
        sub = sweep_df[sweep_df["k_fac"] == kf]
        s_errs = [sub[sub["PFAS_Name"] == p]["pct_err"].values[0]
                  if len(sub[sub["PFAS_Name"] == p]) and pd.notna(sub[sub["PFAS_Name"] == p]["pct_err"].values[0]) else np.nan
                  for p in sulfs_obs]
        c_errs = [sub[sub["PFAS_Name"] == p]["pct_err"].values[0]
                  if len(sub[sub["PFAS_Name"] == p]) and pd.notna(sub[sub["PFAS_Name"] == p]["pct_err"].values[0]) else np.nan
                  for p in carbs_obs]
        s_str = " ".join(f"{e:>+8.0f}%" if pd.notna(e) else f"{'n/a':>9}" for e in s_errs)
        c_str = " ".join(f"{e:>+8.0f}%" if pd.notna(e) else f"{'n/a':>9}" for e in c_errs)
        marker = " ◀ default" if kf == K_FAC_DEFAULT else ""
        print(f"  {kf:>8.3f} | {s_str}  |  {c_str}{marker}")

    # ── Identify best K_FAC for each sulfonate (minimises |pct_err|) ─
    print(f"\n  Best K_FAC per sulfonate (minimises |% error|):")
    for pfas in sulfs_obs:
        sub = sweep_df[sweep_df["PFAS_Name"] == pfas].dropna(subset=["pct_err"])
        if sub.empty:
            continue
        best_idx  = sub["pct_err"].abs().idxmin()
        best_row  = sub.loc[best_idx]
        v13_ref   = {"PFOS": -95, "PFHxS": -88, "PFBS": -72}.get(pfas, np.nan)
        delta_str = (f"{best_row['pct_err'] - v13_ref:+.0f}pp vs v13.0"
                     if pd.notna(v13_ref) else "")
        print(f"    {pfas}: K_FAC={best_row['k_fac']:.3f}  "
              f"% err={best_row['pct_err']:+.0f}%  {delta_str}")

    # ── Heatmap plot ──────────────────────────────────────────────────
    if path is not None:
        pivot = sweep_df.pivot_table(
            index="k_fac", columns="PFAS_Name", values="pct_err")
        pivot = pivot.sort_index(ascending=True)

        fig, axes = plt.subplots(1, 2, figsize=(max(10, len(pivot.columns) * 1.0), 6))

        # Left: full heatmap
        sns.heatmap(pivot, annot=True, fmt=".0f", center=0,
                    cmap="RdBu_r", linewidths=0.5, ax=axes[0],
                    cbar_kws={"label": "% error vs observed BCF"})
        axes[0].set_title(
            "Arnot-Gobas Sensitivity: Protein-Facilitated Gill Uptake (K_FAC)\n"
            f"k1 = k1_passive + K_FAC × Ka_albumin × [alb] × GV  "
            f"(K_FAC_DEFAULT={K_FAC_DEFAULT})"
        )
        axes[0].set_xlabel("PFAS")
        axes[0].set_ylabel("K_FAC (gill transfer efficiency)\n(0.0 = pure passive diffusion)")

        # Right: % error vs K_FAC line chart for sulfonates with observed data
        ax2 = axes[1]
        colors_line = {"PFOS": "steelblue", "PFHxS": "coral", "PFBS": "forestgreen"}
        for pfas in sulfs_obs:
            sub = sweep_df[sweep_df["PFAS_Name"] == pfas].dropna(subset=["pct_err"])
            if sub.empty:
                continue
            ax2.plot(sub["k_fac"], sub["pct_err"],
                     marker="o", lw=2, label=pfas,
                     color=colors_line.get(pfas, "gray"))
        ax2.axhline(0, color="black", lw=1.0, ls="--", label="0% error (perfect)")
        ax2.axvline(K_FAC_DEFAULT, color="gray", lw=1.0, ls=":", label=f"K_FAC_DEFAULT={K_FAC_DEFAULT}")
        ax2.set_xlabel("K_FAC (gill transfer efficiency)")
        ax2.set_ylabel("% error vs observed BCF")
        ax2.set_title("Sulfonate % Error vs K_FAC\n(zero-crossing = model matches observed)")
        ax2.legend(fontsize=8)
...
# ═══════════════════════════════════════════════════════════════════════════
# v15.0 — Two-compartment toxicokinetic model (Option B)
# ═══════════════════════════════════════════════════════════════════════════

def compute_two_comp_bcf(pfas_features_df, q_blood=None, c_alb_tissue_g_l=None,
                         nlom_factor=None):
    """
    Steady-state two-compartment toxicokinetic BCF for PFAS in fish.

    Compartment 1 — blood/plasma (V1):
      Kblood = Ka_albumin × [albumin]_blood_mol_L + VW_BLOOD
      No lipid or NLOM: PFAS bind to albumin in plasma, not triglycerides.

    Compartment 2 — peripheral tissue (V2):
      Ktissue = Ka_albumin × [albumin]_tissue_mol_L × FW_PROTEIN
                + 0.035 × Koc   (NLOM term; Gobas et al. 2003)
                + VW_TISSUE
      VL × Kow excluded: PFAS accumulate in protein-rich tissue, not lipid
      (Conder et al. 2008 Table 3: protein >> lipid for PFAS).

    Rate constants:
      k1  = k1_passive + k1_fac  (sulfonates; same K_FAC_DEFAULT as v14.1)
      k1  = k1_passive            (carboxylates)
      k2  = k1 / Kblood
      k12 = Q_BLOOD / (V1 × Kblood)   blood → tissue
      k21 = Q_BLOOD / (V2 × Ktissue)  tissue → blood
      ke  = Gd × (1 − Ed) × Kfood / Kblood
      kg  = growth dilution (both compartments)

    Steady-state 2×2 system (Cw = 1):
      0 = k1 − (k2 + k12 + ke + km + kg) × C1 + k21 × C2
      0 = k12 × C1 − (k21 + kg) × C2
    Solved by Cramer's rule.

    Whole-body BCF = V1 × C1_ss + V2 × C2_ss
    """
    if q_blood is None:
        q_blood = Q_BLOOD
    c_alb_tissue_mol_l = (
        (c_alb_tissue_g_l / MW_ALBUMIN_2COMP)
        if c_alb_tissue_g_l is not None
        else C_ALBUMIN_TISSUE_MOL_L
    )

    # nlom_factor can be passed as a scalar (sweep) or left None to use
    # the class-specific dict. Scalar override applies to all compounds.
    _nlom_is_dict = isinstance(NLOM_TISSUE_FACTOR, dict)
    _nlom_override = nlom_factor  # None means use class-specific default

    # Arnot-Gobas fish physiology (unchanged from v14.x)
    Ew = 0.50; GV = 400.0; Ed = 0.75; Gd = 0.025
    kg = 0.001; km = 0.0
    Vl_food = 0.05; Vn_food = 0.09; Vw_food = 0.72

    ka_median_log = pfas_features_df["Ka_albumin"].dropna().median()
    ...
    for _, row in pfas_features_df.iterrows():
        ...
        koc        = row.get("Koc", np.nan)
        pfas_class = row["PFAS_Class"]
        ka_log     = row.get("Ka_albumin", np.nan)
        kow        = 10.0 ** log_kow

        if pd.isna(ka_log):
            ka_log     = ka_median_log
            ka_imputed = True
        else:
            ka_imputed = False
        ka_mol_l = 10.0 ** ka_log

        # ── Partition coefficients ────────────────────────────────────
        # Kblood: plasma albumin + plasma water; no lipid/NLOM
        Kblood = ka_mol_l * C_ALBUMIN_BLOOD_MOL_L + VW_BLOOD

        # Ktissue NLOM: class-specific factor unless a scalar override
        # is passed (sweep mode). Sulfonates use 0.002; carboxylates 0.010.
        if _nlom_override is not None:
            _nf = _nlom_override   # scalar sweep value
        elif _nlom_is_dict:
            _nf = NLOM_TISSUE_FACTOR.get(pfas_class, 0.002)
        else:
            _nf = NLOM_TISSUE_FACTOR   # scalar constant (legacy)

        if pd.notna(koc) and koc > 0:
            Kd_nlom = _nf * koc
        else:
            Kd_nlom = VN_TISSUE * _nf * kow
        Ktissue = (ka_mol_l * c_alb_tissue_mol_l * FW_PROTEIN
                   + Kd_nlom
                   + VW_TISSUE)

        # Kfood — prey partition (protein + NLOM + water; no Kow lipid)
        Ka_tissue_food = ka_mol_l * c_alb_tissue_mol_l * FW_PROTEIN
        if pd.notna(koc) and koc > 0:
            Kfood = Ka_tissue_food + Kd_nlom + Vw_food
        else:
            Kfood = Ka_tissue_food + Vw_food

        # ── Rate constants ─────────────────────────────────────────────
        p_mem = P_MEM_CORRECTION.get(pfas_class, 1.0)
        mem_r = 1.0 / (5.49e7 * (kow ** -0.670) + 1.0)
        k1_passive = Ew * GV * mem_r * p_mem
        # Compound-specific K_FAC threshold (Finding 20):
        # PFBS Ka_albumin = 10^2.90 = 794 L/mol — too low to support
        # meaningful facilitated transport. The class-conditional sulfonate
        # flag is too coarse; only apply k1_fac where Ka_albumin > 10^3.5
        # (empirical threshold from Ng & Hungerbühler 2013 data range).
        KA_FAC_THRESHOLD = 10 ** 3.5   # ≈ 3162 L/mol
        if pfas_class == "Sulfonate" and not ka_imputed and ka_mol_l > KA_FAC_THRESHOLD:
            k1_fac = K_FAC_DEFAULT * ka_mol_l * C_ALBUMIN_BLOOD_MOL_L * GV
        else:
            k1_fac = 0.0
        k1 = k1_passive + k1_fac

        k2  = k1 / Kblood
        k12 = q_blood / (V1 * Kblood)
        k21 = q_blood / (V2 * Ktissue)
        ke  = Gd * (1.0 - Ed) * (Kfood / Kblood)
        kd_diet = Ed * Gd * (Kfood / Kblood)

        # ── Steady-state solution (Cramer's rule) ──────────────────────
        a11 = -(k2 + k12 + ke + km + kg)
        a12 =  k21
        a21 =  k12
        a22 = -(k21 + kg)
        ...
        det = a11 * a22 - a12 * a21
        if abs(det) < 1e-30:
            C1_ss = C2_ss = bcf_2comp = np.nan
        else:
            C1_ss     = (b1 * a22 - b2 * a12) / det
            C2_ss     = (a11 * b2 - a21 * b1) / det
            bcf_2comp = V1 * C1_ss + V2 * C2_ss

        log_bcf_2comp = (np.log10(bcf_2comp)
                         if pd.notna(bcf_2comp) and bcf_2comp > 0 else np.nan)

        rows.append({
            "PFAS_Name": pfas_name, "PFAS_Class": pfas_class,
            "Ka_albumin": ka_log, "Ka_imputed": ka_imputed,
            "Kblood": Kblood, "Ktissue": Ktissue,
            "k1_passive": k1_passive, "k1_fac": k1_fac, "k1": k1,
            "k2": k2, "k12": k12, "k21": k21,
            "ke": ke, "kd_diet": kd_diet, "kg": kg, "km": km,
            "q_blood": q_blood,
            "C1_ss": C1_ss, "C2_ss": C2_ss,
            "BCF_2comp": bcf_2comp, "log_BCF_2comp": log_bcf_2comp,
        })

    return pd.DataFrame(rows)


def run_two_comp_nlom_sensitivity(pfas_features_df, observed_bcf_df,
                                   sweep_values=None, path=None):
    """
    v15.0 (revised): Sensitivity sweep of the NLOM tissue sorption coefficient.

    The tissue albumin sweep (first attempt) proved invariant because
    Kd_nlom = 0.035 × Koc dominates Ktissue for high-Koc sulfonates:
      PFOS:  0.035 × 2100 = 73.5  vs  albumin term ≈ 0.93  → NLOM is 98% of Ktissue
      PFHxS: 0.035 × 560  = 19.6  vs  albumin term ≈ 0.25  → NLOM is 98% of Ktissue

    The 0.035 coefficient was calibrated on sediment organic carbon (Gobas
    et al. 2003), not fish muscle tissue. This sweep tests whether a
    tissue-appropriate NLOM coefficient (expected to be 5–100× lower based
    on Kelly et al. 2004 and Conder et al. 2008) brings the two-compartment
    BCF into agreement with observed values.

    Finding 20 test: the value of nlom_factor where the sulfonate triad
    (PFOS, PFHxS, PFBS) simultaneously crosses zero % error — if such a
    value exists within a physically plausible range — is the publishable
    result of the Option B investigation.
    """
    if sweep_values is None:
        sweep_values = NLOM_TISSUE_FACTOR_SWEEP

    obs_median = (observed_bcf_df.groupby("PFAS_Name")["log_BCF"]
                  .median().rename("obs_log_bcf"))

    records = []
    for nf in sweep_values:
        df = compute_two_comp_bcf(pfas_features_df, nlom_factor=nf)
        for _, r in df.iterrows():
            obs = obs_median.get(r["PFAS_Name"], np.nan)
            pct_err = ((r["BCF_2comp"] - 10**obs) / 10**obs * 100
                       if pd.notna(obs) and pd.notna(r["BCF_2comp"]) and r["BCF_2comp"] > 0
                       else np.nan)
            records.append({
                "nlom_factor":   nf,
                "PFAS_Name":     r["PFAS_Name"],
                "PFAS_Class":    r["PFAS_Class"],
                "BCF_2comp":     r["BCF_2comp"],
                "log_BCF_2comp": r["log_BCF_2comp"],
                "Kblood":        r["Kblood"],
                "Ktissue":       r["Ktissue"],
                "pct_err":       pct_err,
            })
    ...
    sulfonates   = [p for p in sweep_df[sweep_df["PFAS_Class"]=="Sulfonate"]["PFAS_Name"].unique()
                    if pd.notna(obs_median.get(p, np.nan))]
    carboxylates = [p for p in sweep_df[sweep_df["PFAS_Class"]=="Carboxylate"]["PFAS_Name"].unique()
                    if pd.notna(obs_median.get(p, np.nan))]

    print("\n[2-Comp NLOM Sweep] Tissue NLOM sorption coefficient sensitivity (v15.0)")
    print(f"  Kd_nlom = nlom_factor × Koc  (replaces hardcoded 0.035 sediment value)")
    print(f"  Kblood fixed ([alb]_blood={C_ALBUMIN_BLOOD_G_L} g/L)")
    print(f"  Finding 20 test: nlom_factor where sulfonate triad simultaneously fits\n")

    header = f"  {'nlom_f':>8} | " + " ".join(f"{p:>8}" for p in sulfonates) + \
             "  |  " + " ".join(f"{p:>8}" for p in carboxylates)
    print(header)
    print("  " + "-" * (len(header) - 2))

    for nf in sweep_values:
        sub = sweep_df[sweep_df["nlom_factor"] == nf]
        s_errs = [sub[sub["PFAS_Name"]==p]["pct_err"].values[0]
                  if len(sub[sub["PFAS_Name"]==p]) else np.nan for p in sulfonates]
        c_errs = [sub[sub["PFAS_Name"]==p]["pct_err"].values[0]
                  if len(sub[sub["PFAS_Name"]==p]) else np.nan for p in carboxylates]
        s_str = " ".join(f"{e:>+8.0f}%" if pd.notna(e) else f"{'n/a':>9}" for e in s_errs)
        c_str = " ".join(f"{e:>+8.0f}%" if pd.notna(e) else f"{'n/a':>9}" for e in c_errs)
        marker = " ◀ default" if nf == NLOM_TISSUE_FACTOR else ""
        print(f"  {nf:>8.4f} | {s_str}  |  {c_str}{marker}")

    # Identify best nlom_factor per sulfonate
    print(f"\n  Best nlom_factor per sulfonate (minimises |% error|):")
    for pfas in sulfonates:
        sub = sweep_df[sweep_df["PFAS_Name"]==pfas].dropna(subset=["pct_err"])
        if sub.empty:
            continue
        best = sub.loc[sub["pct_err"].abs().idxmin()]
        print(f"    {pfas}: nlom_factor={best['nlom_factor']:.4f}  "
              f"% err={best['pct_err']:+.0f}%  Ktissue={best['Ktissue']:.2f}")

    if path is not None:
        pivot = sweep_df.pivot_table(
            index="nlom_factor", columns="PFAS_Name", values="pct_err")
        pivot = pivot.sort_index(ascending=False)

        fig, axes = plt.subplots(1, 2, figsize=(max(10, len(pivot.columns)), 5))
        sns.heatmap(pivot, annot=True, fmt=".0f", center=0,
                    cmap="RdBu_r", linewidths=0.5, ax=axes[0],
                    cbar_kws={"label": "% error vs observed BCF"})
        axes[0].set_title(
            "Two-Compartment Sensitivity: NLOM Tissue Sorption Coefficient\n"
            f"Kd_nlom = nlom_factor × Koc  (sediment baseline = 0.035)"
        )
        axes[0].set_xlabel("PFAS")
        axes[0].set_ylabel("NLOM tissue factor\n(0.035 = Gobas sediment baseline)")

        ax2 = axes[1]
        colors_line = {"PFOS": "steelblue", "PFHxS": "coral", "PFBS": "forestgreen"}
        for pfas in sulfonates:
            sub = sweep_df[sweep_df["PFAS_Name"]==pfas].dropna(subset=["pct_err"])
            ax2.plot(sub["nlom_factor"], sub["pct_err"], marker="o", lw=2,
                     label=pfas, color=colors_line.get(pfas, "gray"))
        ax2.axhline(0, color="black", lw=1, ls="--", label="0% error")
        _nlom_default_str = (str(NLOM_TISSUE_FACTOR) if isinstance(NLOM_TISSUE_FACTOR, dict)
                             else f"{NLOM_TISSUE_FACTOR}")
        ax2.axvline(NLOM_TISSUE_FACTOR.get("Sulfonate", 0.002)
                    if isinstance(NLOM_TISSUE_FACTOR, dict)
                    else NLOM_TISSUE_FACTOR,
                    color="gray", lw=1, ls=":", label=f"default={_nlom_default_str}")
        ax2.set_xlabel("NLOM tissue factor")
        ax2.set_ylabel("% error vs observed BCF")
        ax2.set_title("Sulfonate % Error vs NLOM Tissue Factor\n"
                      "(zero-crossing = model matches observed)")
...
def print_two_comp_summary(df_2comp, observed_bcf_df, df_1comp=None):
    """Console table: 2-comp BCF vs 1-comp vs observed, with Kblood/Ktissue."""
    obs_median = (observed_bcf_df.groupby("PFAS_Name")["log_BCF"]
                  .agg(["median", "count"])
                  .reset_index()
                  .rename(columns={"median": "obs_log_bcf", "count": "n_obs"})) \
    ...
    merged = df_2comp.merge(obs_median, on="PFAS_Name", how="left")
    if df_1comp is not None:
        merged = merged.merge(
            df_1comp[["PFAS_Name", "log_BCF_AG"]].rename(
                columns={"log_BCF_AG": "log_BCF_1comp"}),
            on="PFAS_Name", how="left")

    print("\n[Two-Comp v15.0] Steady-state BCF — two-compartment TK model")
    print(f"  V1={V1} (blood), V2={V2:.2f} (tissue)")
    print(f"  [alb]_blood={C_ALBUMIN_BLOOD_G_L} g/L → {C_ALBUMIN_BLOOD_MOL_L:.2e} mol/L")
    print(f"  [alb]_tissue={C_ALBUMIN_TISSUE_G_L} g/L → {C_ALBUMIN_TISSUE_MOL_L:.2e} mol/L")
    nlom_str = (str(NLOM_TISSUE_FACTOR) if isinstance(NLOM_TISSUE_FACTOR, dict)
                else f"{NLOM_TISSUE_FACTOR}")
    print(f"  Q_blood={Q_BLOOD} L/kg/d (Farrell 1991)")
    print(f"  NLOM_TISSUE_FACTOR={nlom_str}\n")

    hdr = (f"  {'PFAS':<8} {'log2C':>6}  {'BCF2C':>7}  "
           f"{'log1C':>6}  {'logObs':>7}  {'n':>4}  "
           f"{'%err2C':>7}  {'%err1C':>7}  "
           f"{'Kblood':>7}  {'Ktiss':>7}  verdict")
    print(hdr)
    print("  " + "-" * (len(hdr) - 2))

    for _, r in merged.sort_values("log_BCF_2comp", ascending=False,
                                    na_position="last").iterrows():
        log2c = f"{r['log_BCF_2comp']:.3f}" if pd.notna(r.get("log_BCF_2comp")) else "n/a"
        bcf2c = f"{r['BCF_2comp']:.1f}"     if pd.notna(r.get("BCF_2comp"))     else "n/a"
        log1c = f"{r['log_BCF_1comp']:.3f}" if pd.notna(r.get("log_BCF_1comp")) else "n/a"
        obs_s = f"{r['obs_log_bcf']:.3f}"   if pd.notna(r.get("obs_log_bcf"))   else "no data"
        n_s   = f"{int(r['n_obs'])}"         if pd.notna(r.get("n_obs"))         else "—"
        kb_s  = f"{r['Kblood']:.2f}"        if pd.notna(r.get("Kblood"))        else "n/a"
        kt_s  = f"{r['Ktissue']:.2f}"       if pd.notna(r.get("Ktissue"))       else "n/a"

        if pd.notna(r.get("obs_log_bcf")) and pd.notna(r.get("BCF_2comp")):
            bcf_obs = 10 ** r["obs_log_bcf"]
            pe2c    = (r["BCF_2comp"] - bcf_obs) / bcf_obs * 100
            pe2c_s  = f"{pe2c:+.0f}%"
            pe1c_s  = "n/a"
            if pd.notna(r.get("log_BCF_1comp")):
                pe1c   = (10**r["log_BCF_1comp"] - bcf_obs) / bcf_obs * 100
                pe1c_s = f"{pe1c:+.0f}%"
                delta  = abs(pe2c) - abs(pe1c)
                verdict = ("IMPROVED" if delta < -10 else
                           "DEGRADED" if delta >  10 else "≈unchanged")
            else:
                verdict = "no 1-comp ref"
        else:
            pe2c_s = pe1c_s = "n/a"
            verdict = "no observed"

        print(f"  {r['PFAS_Name']:<8} {log2c:>6}  {bcf2c:>7}  "
              f"{log1c:>6}  {obs_s:>7}  {n_s:>4}  "
              f"{pe2c_s:>7}  {pe1c_s:>7}  "
              f"{kb_s:>7}  {kt_s:>7}  {verdict}")

    # Finding 20 verdict
    sulfonate_v14_refs = {"PFOS": -25, "PFHxS": +34, "PFBS": -68}
    improved = []; degraded = []; unchanged = []
    for _, r in merged.iterrows():
        if r["PFAS_Class"] != "Sulfonate":
            continue
        if pd.isna(r.get("obs_log_bcf")) or pd.isna(r.get("BCF_2comp")):
            continue
        pe2c    = (r["BCF_2comp"] - 10**r["obs_log_bcf"]) / 10**r["obs_log_bcf"] * 100
        pe1c_ref = sulfonate_v14_refs.get(r["PFAS_Name"], np.nan)
        if pd.notna(pe1c_ref):
            delta = abs(pe2c) - abs(pe1c_ref)
            bucket = improved if delta < -10 else (degraded if delta > 10 else unchanged)
            bucket.append((r["PFAS_Name"], pe1c_ref, pe2c, delta))

    print(f"\n  [Finding 20 — Two-compartment vs v14.1 single-compartment]")
    print(f"  v14.1 baseline: PFOS −25%, PFHxS +34%, PFBS −68%")
    print(f"  *** CALIBRATION CAVEAT: NLOM_TISSUE_FACTOR and K_FAC were selected")
    print(f"  *** to minimise error for these same compounds. Residuals below are")
    print(f"  *** therefore in-sample fits, NOT independent validation. With only")
    print(f"  *** 6 observed BCF values and more free parameters than data points,")
    print(f"  *** interpret as 'what the model can reconcile with available data',")
    print(f"  *** not as confirmation that the two-compartment architecture is correct.")
    for label, lst in [("IMPROVED (in-sample)", improved), ("DEGRADED", degraded), ("≈unchanged", unchanged)]:
        if lst:
            print(f"  {label}: " + ", ".join(
                f"{n} {r:+.0f}%→{e:+.0f}% (Δ={d:+.0f}pp)" for n,r,e,d in lst))
    if improved and not degraded and all(abs(e) < 25 for _,_,e,_ in improved+unchanged):
        print(f"\n  OUTCOME: Two-compartment fits the sulfonate triad (in-sample).")
        print(f"  Independent validation requires held-out compounds or literature params.")
    elif improved and not degraded:
        print(f"\n  OUTCOME: Two-compartment fits some sulfonates better (in-sample).")
        print(f"  PFHxS residual error (+34%→+128% range) indicates over-parameterisation.")
    elif degraded:
        print(f"\n  OUTCOME: Two-compartment does not improve sulfonates vs v14.1.")


def run_two_comp_model(pfas_features_df, observed_bcf_df, output_dir,
                       ag_df_1comp=None):
    """
    v15.0: Run the full two-compartment model analysis.
      1. Default Q_BLOOD run + console summary (Finding 20)
      2. Tissue albumin sweep (primary calibration parameter)
      3. Comparison plot vs single-compartment vs observed

    Returns dict with df_2comp and sweep_df.
    """
    print("\n" + "─" * 60)
    print("  [Two-Compartment Model v15.0] Option B")
    print(f"  Blood: V1={V1}, [alb]={C_ALBUMIN_BLOOD_G_L} g/L")
    print(f"  Tissue: V2={V2:.2f}, [alb]={C_ALBUMIN_TISSUE_G_L} g/L (Barber 2003)")
    print(f"  Q_blood={Q_BLOOD} L/kg/d (Farrell 1991)")
    print("─" * 60)

    df_2comp = compute_two_comp_bcf(pfas_features_df)
    print_two_comp_summary(df_2comp, observed_bcf_df, df_1comp=ag_df_1comp)

    sweep_df = run_two_comp_nlom_sensitivity(
        pfas_features_df, observed_bcf_df,
        path=output_dir + "two_comp_nlom_sensitivity.png"
    )

    # Three-panel comparison plot
    if ag_df_1comp is not None and len(observed_bcf_df) > 0:
        obs_median = (observed_bcf_df.groupby("PFAS_Name")["log_BCF"]
                      .agg(["median", "count"])
                      .reset_index()
                      .rename(columns={"median": "obs_log_bcf", "count": "obs_n"}))
        df = df_2comp.merge(obs_median, on="PFAS_Name", how="left")
        df = df.merge(
            ag_df_1comp[["PFAS_Name", "log_BCF_AG", "BCF_AG"]].rename(
                columns={"log_BCF_AG": "log_BCF_1comp", "BCF_AG": "BCF_1comp"}),
            on="PFAS_Name", how="left")
        df = df.dropna(subset=["log_BCF_2comp"]).sort_values("log_BCF_2comp")

        names = df["PFAS_Name"].tolist()
        y = np.arange(len(df))
        w = 0.25

        fig, axes = plt.subplots(1, 2, figsize=(14, max(6, len(df) * 0.7)))

        ax1 = axes[0]
        ax1.barh([i - w for i in y], df["log_BCF_1comp"],
                 w, color="lightsteelblue", alpha=0.9, label="1-comp v14.1")
        ax1.barh([i for i in y], df["log_BCF_2comp"],
                 w, color="steelblue", alpha=0.9, label="2-comp v15.0")
        has_obs = df["obs_log_bcf"].notna()
        if has_obs.any():
            ax1.barh(np.array([i + w for i in y])[has_obs.values],
                     df.loc[has_obs, "obs_log_bcf"],
                     w, color="coral", alpha=0.9, label="Observed median")
        ax1.set_yticks(list(y)); ax1.set_yticklabels(names)
        ax1.set_xlabel("log₁₀ BCF")
        ax1.set_title("BCF: 1-comp vs 2-comp vs Observed")
        ax1.axvline(0, color="black", lw=0.6); ax1.legend(fontsize=7)

        ax2 = axes[1]
        sub = df[df["obs_log_bcf"].notna() & df["BCF_2comp"].notna()
                 & df["BCF_1comp"].notna()].copy()
        if not sub.empty:
            bcf_obs    = 10 ** sub["obs_log_bcf"]
            pe_1c = (sub["BCF_1comp"] - bcf_obs) / bcf_obs * 100
            pe_2c = (sub["BCF_2comp"] - bcf_obs) / bcf_obs * 100
            sub_y = np.arange(len(sub))
            ax2.barh([i - w/2 for i in sub_y], pe_1c, w,
                     color="lightsteelblue", alpha=0.9, label="1-comp")
            ax2.barh([i + w/2 for i in sub_y], pe_2c, w,
                     color="steelblue", alpha=0.9, label="2-comp")
            ax2.axvline(0, color="black", lw=0.8)
            ax2.set_yticks(list(sub_y))
            ax2.set_yticklabels(sub["PFAS_Name"].tolist())
            ax2.set_xlabel("% error vs observed BCF")
            ax2.set_title("% Error: 1-comp vs 2-comp\n(zero = perfect)")
            ax2.legend(fontsize=7)

        plt.tight_layout()
        plt.savefig(output_dir + "arnot_gobas_2comp_bcf.png", dpi=150)
        plt.close()
        print(f"[Output] → {output_dir}arnot_gobas_2comp_bcf.png")

    print("─" * 60)
    return {"df_2comp": df_2comp, "sweep_df": sweep_df}

def main():
    print("\n" + "═" * 60)
    print("  PFAS Bioaccumulation Pipeline v17.0")
    print("  v17.0: Target var fix | BCF fish-only | per-PFAS leak fix | multi-seed R²")
    print("  v15.0: Two-compartment TK model (Option B) — Finding 20")
    print("  Blood [alb]=20 g/L (V1=0.05) | Tissue [alb]=5 g/L (Barber 2003)")
    print("  Sulfonates: k1 = k1_passive + K_FAC × Ka_albumin × [albumin]_blood × GV")
    print(f"  K_FAC_DEFAULT={K_FAC_DEFAULT}  (Ng & Hungerbühler 2013, sulfonates only)")
    print("═" * 60 + "\n")
    ...
    if os.path.exists(NHANES_PATH):
        ...
        # v17.0: non-detect audit (mentor feedback point 5a)
        # NHANES reports values at or below LOD; rows with Concentration_ng_g==0
        # or negative are non-detects. Standard NHANES imputation: LOD/sqrt(2).
        # This pipeline currently drops them (harmonize keeps only > 0).
        # Count here for transparency; full imputation requires the LOD column.
        if "Concentration_ng_g" in nhanes.columns:
            n_nd = (nhanes["Concentration_ng_g"] <= 0).sum()
            n_nan = nhanes["Concentration_ng_g"].isna().sum()
            n_valid = (nhanes["Concentration_ng_g"] > 0).sum()
            print(f"[NHANES] Non-detects (value ≤ 0): {n_nd:,}  |  "
                  f"Missing: {n_nan:,}  |  Valid (>0): {n_valid:,}")
            if n_nd > 0:
                print(f"[NHANES] WARNING: {n_nd:,} non-detects excluded. "
                      f"This biases mean/median upward, especially for short-chain PFAS. "
                      f"Add LOD column and impute as LOD/√2 for publication.")

        # v17.0: survey weight application (mentor feedback point 5b)
        # NHANES uses a complex stratified cluster design; raw means are biased.
        # WTSB2YR is the 2-year subsample weight for the PFAS lab files.
        # Units note: NHANES reports serum PFAS in ng/mL; serum density ≈ 1 g/mL,
        # so ng/mL ≈ ng/g.  We assume this equivalence explicitly here.
        weight_col = ("WTSB2YR" if "WTSB2YR" in nhanes.columns
                      else "WTSA2YR" if "WTSA2YR" in nhanes.columns
                      else None)
        if weight_col is None:
            print(f"[NHANES] WARNING: Survey weights (WTSB2YR/WTSA2YR) not found. "
                  f"All statistics are unweighted sample means from a complex survey design. "
                  f"NHANES oversamples specific groups — results are NOT nationally "
                  f"representative. Add WTSB2YR column for publication-grade estimates.")
            print(f"[NHANES] Units note: serum PFAS in ng/mL ≈ ng/g (assuming serum density "
                  f"≈ 1 g/mL). This approximation is standard in PFAS biomonitoring literature.")
        else:
            # Compute weighted means per PFAS and report alongside unweighted
            print(f"[NHANES] Survey weights found in '{weight_col}' — computing weighted statistics.")
            print(f"[NHANES] Units note: serum PFAS in ng/mL ≈ ng/g (serum density ≈ 1 g/mL).")
            if "Concentration_ng_g" in nhanes.columns and "PFAS_Name" in nhanes.columns:
                weighted_stats = []
                for pfas, grp in nhanes[nhanes["Concentration_ng_g"] > 0].groupby("PFAS_Name"):
                    w = grp[weight_col].fillna(0)
                    c = grp["Concentration_ng_g"]
                    w_sum = w.sum()
                    if w_sum > 0:
                        w_mean = (c * w).sum() / w_sum
                        uw_mean = c.mean()
                        weighted_stats.append({
                            "PFAS_Name": pfas, "n": len(grp),
                            "unweighted_mean": uw_mean, "weighted_mean": w_mean,
                            "bias_pct": (w_mean - uw_mean) / uw_mean * 100 if uw_mean else np.nan,
                        })
                if weighted_stats:
                    ws_df = pd.DataFrame(weighted_stats)
                    print(f"\n[NHANES] Weighted vs unweighted means (ng/g serum):")
                    print(f"  {'PFAS':<8} {'n':>6}  {'Unwt mean':>10}  {'Wt mean':>10}  {'bias%':>7}")
                    print(f"  {'-'*8}  {'-'*6}  {'-'*10}  {'-'*10}  {'-'*7}")
                    for _, r in ws_df.iterrows():
                        bias_s = f"{r['bias_pct']:+.1f}%" if pd.notna(r['bias_pct']) else "n/a"
                        print(f"  {r['PFAS_Name']:<8}  {int(r['n']):>6}  "
                              f"{r['unweighted_mean']:>10.3f}  {r['weighted_mean']:>10.3f}  {bias_s:>7}")
                    print(f"  [Note: weighted means are the nationally representative estimates]")
    ...
    print(f"[Models] Distinct chemistry profiles (effective N): {pfas_id.nunique()}")
    print(f"[Models] NOTE: R² confidence appropriate to {pfas_id.nunique()} compounds, "
          f"not {len(X):,} rows — features are constant within a compound.\n")

    # v17.0: multi-seed R² before committing to the single seed=42 split.
    # Mentor feedback: single-split R² with no uncertainty is misleading.
    print("[MultiSeed] Evaluating R² stability across 20 random seeds...")
    ms_results = evaluate_rf_multiseed(X, y, groups, n_seeds=20, label="Concentration RF")
    ...
    # Observed BCF records — restricted to confirmed Fish records only.
    # v17.0: "Other" group removed. "Other" is whatever fell through the 17-entry
    # SPECIES_GROUP_MAP — a grab bag of invertebrates, algae, unmatched taxa.
    # PFHxS has zero fish records (Finding 6), so its prior "observed BCF" came
    # entirely from Other, making the mechanistic fish model comparison meaningless.
    # n and IQR are reported per compound so sparsity is visible.
    observed_bcf = dataset[
        dataset["log_BCF"].notna() & (dataset["Species_Group"] == "Fish")
    ][["PFAS_Name", "log_BCF"]].copy()
    n_bcf_by_pfas = observed_bcf.groupby("PFAS_Name")["log_BCF"].agg(["count", "median",
        lambda x: x.quantile(0.25), lambda x: x.quantile(0.75)]).round(3)
    n_bcf_by_pfas.columns = ["n", "median", "q25", "q75"]
    print(f"\n[BCF] Observed fish-only BCF records per PFAS:")
    for pfas, row in n_bcf_by_pfas.iterrows():
        print(f"  {pfas:<10} n={int(row['n']):>4}  median={row['median']:.3f}  "
              f"IQR=[{row['q25']:.3f}, {row['q75']:.3f}]"
              + ("  ← sparse" if row['n'] < 5 else ""))
    ...
    # v14.0: K_FAC sensitivity sweep — Option A protein-facilitated gill uptake.
    # Tests whether augmenting k1 with a Ka_albumin-weighted facilitated-transport
    # term closes the sulfonate BCF gap that Kfish tuning (Finding 18), Kprot
    # tuning (Finding 16), and P_mem tuning (Finding 17) could not fix.
    # This is the final single-compartment mechanistic hypothesis available within
    # the Arnot-Gobas framework. K_FAC=0.0 is the v13.0 baseline (pure passive
    # diffusion); K_FAC in [0.005, 0.02] is the empirically grounded range from
    # Ng & Hungerbühler (2013) for rainbow trout.
    kfac_sweep_df = run_arnot_gobas_kfac_sensitivity(
        chem_df, observed_bcf,
        path=OUTPUT_DIR + "arnot_gobas_kfac_sensitivity.png"
    )

    # v15.0: Two-compartment model (Option B) — Finding 20
    # Tests whether separating blood albumin pool (20 g/L) from peripheral
    # tissue (5 g/L interstitial) simultaneously resolves PFOS/PFHxS/PFBS.
    # Includes tissue albumin sweep as the primary calibration parameter.
    two_comp_results = run_two_comp_model(
        chem_df, observed_bcf, OUTPUT_DIR,
        ag_df_1comp=ag_df
    )
    ...
    # ── LEAVE-ONE-COMPOUND-OUT (LOCO) ────────────────────────────────
    # This is the headline evaluation: can chemistry predict an unseen
    # compound's accumulation? Pooled random-split R² is inflated because
    # CONC_FEATURES are identical across all rows of the same PFAS, so
    # train/test contamination is structural, not accidental. LOCO removes
    # that contamination entirely and gives the scientifically honest number.
    loco_results = run_loco_validation(dataset, OUTPUT_DIR)
    ...
    # ── SUMMARY ─────────────────────────────────────────────────────
    print("\n" + "═" * 60)
    print("  Held-out Test Results — v16.0")
    print("═" * 60)
    ...
    # ── LOCO SUMMARY ────────────────────────────────────────────────
    print(f"\n  [Leave-One-Compound-Out CV — Headline Chemistry R²]")
    print(f"  This is the scientifically honest evaluation of whether chemistry")
    print(f"  features can predict accumulation for an unseen compound.")
    r2_loco = loco_results.get("r2_loco_pooled", np.nan)
    rmse_loco = loco_results.get("rmse_loco_pooled", np.nan)
    if pd.notna(r2_loco):
        print(f"    Pooled LOCO R² : {r2_loco:+.3f}")
        print(f"    Pooled LOCO RMSE: {rmse_loco:.3f}")
        print(f"    (Pooled random-split R²={r2_rf:.3f} for comparison —")
        print(f"     inflated because train/test share identical feature rows)")
    loco_df_ok = loco_results["loco_df"][loco_results["loco_df"]["note"] == "ok"]
    if not loco_df_ok.empty:
        best_loco  = loco_df_ok.loc[loco_df_ok["R2"].idxmax()]
        worst_loco = loco_df_ok.loc[loco_df_ok["R2"].idxmin()]
        print(f"    Best  LOCO fold: {best_loco['PFAS']} (R²={best_loco['R2']:+.3f}, n={int(best_loco['n_test'])})")
        print(f"    Worst LOCO fold: {worst_loco['PFAS']} (R²={worst_loco['R2']:+.3f}, n={int(worst_loco['n_test'])})")
    ...
    if len(ag_with_data) > 0:
        observed_bcf_summary = dataset[
            dataset["log_BCF"].notna() & (dataset["Species_Group"] == "Fish")
        ].groupby("PFAS_Name")["log_BCF"].median()  # v17.0: fish-only
        sulfonate_errs = []
        carboxylate_errs = []
        ...
        if sulfonate_errs:
            ...
            for n, e in sulfonate_errs:
                ...
                if ref:
                    delta = e - ref
                    verdict = ("IMPROVED" if abs(e) < abs(ref) - 10
                               else "DEGRADED" if abs(e) > abs(ref) + 10
                               else "unchanged")
    ...
    # ── FINDING 19 — K_FAC sweep result ─────────────────────────────
    print(f"\n  [Finding 19 — Protein-facilitated k1 vs observed BCF (v14.1)]")
    print(f"    K_FAC_DEFAULT={K_FAC_DEFAULT}  (Ng & Hungerbühler 2013 midpoint, sulfonates only)")
    print(f"    Sulfonates: k1 = k1_passive + K_FAC × Ka_albumin × [albumin]_blood × GV")
    print(f"    Carboxylates: k1 = k1_passive (K_FAC=0 fixed)")
    # v12.x: pre-Ka-albumin Kfish (Koc-proxy era) — hardcoded from archived runs.
    v12_refs = {"PFOS": -91, "PFHxS": -85, "PFBS": -84,
                "PFOA": +5,  "PFDA":  +2,  "PFNA": -63}
    if len(ag_with_data) > 0:
        observed_bcf_summary_19 = dataset[
            dataset["log_BCF"].notna() & (dataset["Species_Group"] == "Fish")
        ].groupby("PFAS_Name")["log_BCF"].median()  # v17.0: fish-only

        # v13.0 baseline: Ka-albumin Kfish, K_FAC=0 (pure passive diffusion).
        # Computed live so this column always reflects the current model, not a
        # stale hardcoded dict that drifts when model parameters change.
        ag_v13 = compute_arnot_gobas_bcf(PFAS_FEATURES, k_fac=0.0)
        v13_live = {}
        for _, r13 in ag_v13.iterrows():
            obs13 = observed_bcf_summary_19.get(r13["PFAS_Name"], np.nan)
            if pd.notna(obs13) and pd.notna(r13["BCF_AG"]):
                v13_live[r13["PFAS_Name"]] = (r13["BCF_AG"] - 10**obs13) / 10**obs13 * 100

        print(f"\n    {'PFAS':<8} {'v12.x':>7}  {'v13.0':>7}  {'v14.0':>7}  {'Δ(14-13)':>9}  verdict")
        print(f"    {'-'*8}  {'-'*7}  {'-'*7}  {'-'*7}  {'-'*9}  {'-'*18}")
        for _, r in ag_with_data.iterrows():
            obs = observed_bcf_summary_19.get(r["PFAS_Name"], np.nan)
            if pd.isna(obs):
                continue
            pct_v14 = (r["BCF_AG"] - 10**obs) / 10**obs * 100
            pct_v13 = v13_live.get(r["PFAS_Name"], np.nan)
            pct_v12 = v12_refs.get(r["PFAS_Name"], np.nan)
            delta_13 = pct_v14 - pct_v13 if pd.notna(pct_v13) else np.nan
            if pd.notna(delta_13):
                if abs(pct_v14) < abs(pct_v13) - 5 and abs(pct_v14) < 20:
                    verdict = "RESOLVED"
                elif abs(pct_v14) < abs(pct_v13) - 5:
                    verdict = "IMPROVED"
                elif abs(pct_v14) > abs(pct_v13) + 5:
                    verdict = "DEGRADED"
                else:
                ...
                verdict = "no ref"
            v12_str = f"{pct_v12:+.0f}%" if pd.notna(pct_v12) else "n/a"
            v13_str = f"{pct_v13:+.0f}%" if pd.notna(pct_v13) else "n/a"
            d13_str = f"{delta_13:+.0f}pp" if pd.notna(delta_13) else "n/a"
            print(f"    {r['PFAS_Name']:<8}  {v12_str:>7}  {v13_str:>7}  "
                  f"{pct_v14:>+7.0f}%  {d13_str:>9}  {verdict}")
        # Assess Option A outcome
        sulfonate_v14_errs = [
            (r["PFAS_Name"],
             (r["BCF_AG"] - 10**observed_bcf_summary_19.get(r["PFAS_Name"], np.nan))
             / 10**observed_bcf_summary_19.get(r["PFAS_Name"], np.nan) * 100)
            for _, r in ag_with_data.iterrows()
            if r["PFAS_Class"] == "Sulfonate"
            and pd.notna(observed_bcf_summary_19.get(r["PFAS_Name"], np.nan))
        ]
        if sulfonate_v14_errs:
            all_closed   = all(abs(e) < 25 for _, e in sulfonate_v14_errs)
            all_improved = all(abs(e) < abs(v13_live.get(n, e)) for n, e in sulfonate_v14_errs)
            if all_closed:
                print(f"\n    OUTCOME A: Gap RESOLVED — protein-facilitated k1 explains "
                      f"sulfonate BCF at K_FAC={K_FAC_DEFAULT:.3f}.")
                print(f"    Finding 19: protein-facilitated uptake quantitatively accounts "
                      f"for the sulfonate BCF gap. Single-compartment model is sufficient.")
            elif all_improved:
                print(f"\n    OUTCOME B: Gap PARTIALLY CLOSED — facilitated transport is "
                      f"directionally correct; K_FAC optimization or Option B (two-compartment)")
                print(f"    may further reduce residual error.")
            else:
                print(f"\n    OUTCOME C: Gap NOT CLOSED by K_FAC={K_FAC_DEFAULT:.3f}. "
                      f"See kfac_sweep for optimal K_FAC.")
                print(f"    If no plausible K_FAC closes the gap, Option B (two-compartment) "
                      f"is indicated.")
    print(f"    See arnot_gobas_kfac_sensitivity.png for full K_FAC sweep.")
