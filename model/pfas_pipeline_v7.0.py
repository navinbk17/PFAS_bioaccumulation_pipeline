"""
PFAS Bioaccumulation Data Pipeline v7.0
========================================
Sources: ECOTOX + EPA CompTox + CDC NHANES (2015-2016, 2017-2018)

Changes in v7.0 - Three high-impact leakage/bias fixes
-------------------------------------------------------
FIX 1 - is_human leakage removed from CONC_FEATURES
  is_human had 0.15 feature importance but encodes data source (NHANES vs
  ECOTOX) rather than biology. A simulator user entering a new PFAS chemical
  cannot know "is this human data" — removing it makes the model generalisable.
  Also removed is_fish, is_mammal, is_plant: Trophic_Level already encodes
  species position cleanly without data-source identity flags.

FIX 2 - Duration_days null leakage removed from CONC_FEATURES
  NHANES rows have no exposure duration. Duration_days=NaN was acting as an
  implicit "this is a human row" flag, leaking source identity a second time.
  Removed from features; documented as a known data gap.

FIX 3 - Stratified train/test split by Species_Group
  Previous random split produced highly variable per-group test sizes
  (e.g. only 37 Fish in test). Stratified split ensures each species group
  is proportionally represented in train and test, making per-group R²
  metrics stable and comparable across runs.

  NOTE: These fixes may reduce headline R² from 0.712. That is expected
  and correct — previous number was partially inflated by source leakage.

Changes in v6.6
---------------
- BCF chemistry-only features, Duration_days excluded, BCF coalescing
- BCF per-PFAS baseline, bcf_xgb outputs added

Changes in v6.5
---------------
- XGBoost concentration + BCF models, unified model comparison table

Changes in v6.0
---------------
- Proper 80/20 train/test split, per-species-group metrics, group mean
  baseline, fish/plant chemistry models, NHANES trend plot
...
def main():
    print("\n" + "═" * 60)
    print("  PFAS Bioaccumulation Pipeline v7.0")
    print("═" * 60 + "\n")
    ...
    # ── SUMMARY ─────────────────────────────────────────────────────
    print("\n" + "═" * 60)
    print("  Held-out Test Results (80/20 stratified split) — v7.0")
    print("═" * 60)
