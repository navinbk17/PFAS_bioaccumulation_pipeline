"""
PFAS Bioaccumulation Data Pipeline v8.0
========================================
Sources: ECOTOX + EPA CompTox + CDC NHANES (2015-2016, 2017-2018)

Changes in v8.0 — Prediction confidence intervals
--------------------------------------------------
- RF tree-level variance used to compute per-sample prediction intervals
- 80% and 95% intervals computed for every test prediction
- Interval coverage diagnostics printed (are intervals well-calibrated?)
- Per-species-group interval width analysis (which groups are most uncertain?)
- predict_with_intervals() function exported for simulator use — given any
  new PFAS input, returns point estimate + lower/upper bounds at 80% and 95%
...
FIX 1 - is_human/is_fish/is_mammal/is_plant removed from CONC_FEATURES
  Encoded data source identity rather than biology; broke simulator generalisability.
FIX 2 - Duration_days removed from CONC_FEATURES
  Null for all NHANES rows — implicit human-row flag (second leakage path).
FIX 3 - Stratified train/test split by Species_Group
  Ensures proportional group representation every run.
...
- NHANES 2015-2016 data, nhanes_time_trend.png, chain_length_bcf_scatter.png
- Fish-only and Plant-only chemistry models
...
-------
  ...
  prediction_intervals.png        <- NEW
  interval_coverage.png           <- NEW
...
def main():
    print("\n" + "═" * 60)
    print("  PFAS Bioaccumulation Pipeline v8.0")
    print("═" * 60 + "\n")
    ...
    # ── SUMMARY ─────────────────────────────────────────────────────
    print("\n" + "═" * 60)
    print("  Held-out Test Results (80/20 stratified split) — v8.0")
    print("═" * 60)
